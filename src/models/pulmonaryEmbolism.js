...
export function solvePulmonaryEmbolism({ obstruction = 0 } = {}) {
  // Domain 0–1 on the teaching axis. Non-finite input takes the reference
  // lung; ±Infinity lands on the nearer bound.
  const progress = clampFinite(obstruction, 0);
  ...
  const units = Array.from({ length: PE_UNIT_COUNT }, (_, id) => {
    ...
    const occlusion = clampFinite(obstructedTerritory * PE_UNIT_COUNT - rank, 0);
