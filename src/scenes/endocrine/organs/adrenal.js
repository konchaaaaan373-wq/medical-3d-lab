 ...
 */
export function buildAdrenal({ cortexColor = '#e8c88a', medullaColor = '#9c6bd8', opacity = 0.55 } = {}) {
  ...
  const cortex = new THREE.Mesh(
    shapedSphere({ detail: 6, scale: [0.46, 0.34, 0.38], warp: capWarp }),
    tissueMaterial({ color: cortexColor, roughness: 0.5, opacity })
  );
  cortex.name = 'cortex';

  const medulla = new THREE.Mesh(
    shapedSphere({ detail: 5, scale: [0.24, 0.17, 0.19], warp: capWarp }),
    tissueMaterial({ color: medullaColor, roughness: 0.4, emissiveIntensity: 0.22 })
  );
  ...
  return {
    ...
    cortexPoints: [
      new THREE.Vector3(-0.3, 0.06, 0.16),
      new THREE.Vector3(0.3, 0.06, 0.16),
      new THREE.Vector3(0, 0.1, -0.24),
    ],
    medullaPoint: new THREE.Vector3(0, 0.06, 0),
    anchors: {
      cortex: new THREE.Vector3(-0.85, 0.5, 0.4),
      medulla: new THREE.Vector3(0.75, -0.1, 0.5),
    },
  };
}
