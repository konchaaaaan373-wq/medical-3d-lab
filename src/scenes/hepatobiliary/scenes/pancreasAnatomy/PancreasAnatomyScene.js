...
export class PancreasAnatomyScene extends OrganAnatomyScene {
  ...
  static cameraPose = {
    position: new THREE.Vector3(-0.1, 0.28, 4.9),
    target: new THREE.Vector3(-0.1, -0.02, 0),
  };

  static lightRig = { key: 30, fill: 0.95, rim: 14 };

  /**
   * The gland is long and shallow — the widest whole-gland view fills the frame
   * at an aspect of 1.09, the highest of the six organs. Measured on the gland;
   * the duodenum and spleen beside it want 1.29 and are allowed to crop, which
   * is what makes them context rather than subject.
   */
  static framing = { minHorizontalAspect: 1.15 };

  /** What is drawn for orientation rather than as the subject. */
  static contextTags = ['neighbour'];

  static colorModes = PANCREAS_COLOR_MODES;

  static views = [
    { id: 'anterior', label: 'Anterior', labelJa: '前面', position: [-0.1, 0.28, 4.9], target: [-0.1, -0.02, 0] },
