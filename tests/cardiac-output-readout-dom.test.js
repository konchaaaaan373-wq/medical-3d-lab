...
test('read-out: a phone gets four rows and a way to the rest', async () => {
  await withDocument(async () => {
    ...
    // The scene declares which rows survive a small screen: what was changed,
    // then the same four computed results whatever is moved — output, stroke
    // volume, arterial pressure and the filling pressure that often moves the
    // other way. The stylesheet decides at what width that applies; what is
    ...
    assert.deepEqual(compact, ['開始時（基準）のまま', '心拍出量 CO', '1回拍出量 SV', '平均動脈圧 MAP', '左室充満圧（LVEDP）']);
