...
import { HEART_MODEL_SCOPE } from '../src/data/heartAnatomy.js';
...
test('scope: every route it offers is a scene this catalogue has', () => {
  ...
  for (const entry of HEART_MODEL_SCOPE.next) {
...
test('scope: a scene the release is holding back is dropped, not linked', () => {
  withFakeDom(() => {
    const panel = createModelScopePanel(HEART_MODEL_SCOPE, { isOpen: (slug) => slug === 'heart-failure' });
    const links = findByClass(panel.element, 'scope-next-link');
...
test('scope: with nothing open the section is not there at all', () => {
  ...
  withFakeDom(() => {
    const panel = createModelScopePanel(HEART_MODEL_SCOPE, { isOpen: () => false });
