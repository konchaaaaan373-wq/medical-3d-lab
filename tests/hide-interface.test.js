...
test('the interface is hidden by `visibility`, which a descendant can undo', () => {
  // The heart of L-36. `opacity: 0` on the box around the button cannot be
...
test('and then steps back, so the frame a capture takes is empty', () => {
  // L-33: the fix for L-36 satisfied the bug report and broke the reason the
  ...
  // the hit test and put the reader back where L-36 left them.
