...
- `/api/billing-health` reports only aggregate health and returns HTTP 503 when configuration, real Stripe Product/Price/Portal readiness, webhook processing, scheduled reconciliation, or full customer-sweep coverage is degraded. A bounded batch cannot report green until every known customer has been reconciled, and the oldest customer reconciliation must remain within 24 hours. It verifies Stripe even when no customer exists and caches that non-mutating provider check for five minutes. Netlify rate-limits it per visitor before a request reaches Supabase.
