...
| **`.scope-next-link` / `.scope-next-note`** | `src/styles/ui.css` | scope パネル内の新しい導線。見た目を寄せる場合はここ。DOM 構造は変えないでください（`tests/model-scope-next.test.js` が読んでいます） |
...
2. **model scope に `next` / `nextNote` を足せます。** 「表現していないこと」の直後に
   「この先はどこで見られるか」が出ます。**公開ゲートで閉じているシーンは自動で落ちます。**
   `nextNote` は「別のモデルであって、この標本のその後ではない」を必ず書いてください
