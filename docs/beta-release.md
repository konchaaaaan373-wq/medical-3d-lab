...
**公開βは「3D の解剖」と、名指しで加えた機序シーンです。**
範囲は臓器名では固定しません——**ゲートを通ったシーンから 1 つずつ**開きます
（[ADR 2026-09-14](architecture/adr-2026-09-14-anatomy-beta-by-organ.md)）。

2026-09-22 まで、ここは「解剖モデルだけ」でした。所有者の判断でそのルールを外し、
`BETA_MECHANISM_CANDIDATES` に名指しした機序シーンを 1 本ずつ加えられるように
しています（[ADR 2026-09-22](architecture/adr-2026-09-22-mechanism-scene-in-beta.md)）。
**外したのは「解剖だけ」の部分だけ**で、代用の禁止も、患者向け機序が現行の
臨床レビューを要することも、そのままです。

**何件公開しているかはここに書きません。**
この行は長く「現在公開しているのは 11 件」と書いており、**間違っていました**
——数えると違う数です。`npm run verify:site` が `publishes N` を出します。
