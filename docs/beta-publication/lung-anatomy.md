...
now `natural` instead of `lobes`. A reader looking at the landing page said the heart did not look like an
organ; this scene changed with it, so that an organ is an organ everywhere.
**What that costs here is real**: the lobes are what the scene exists to tell apart, and `lobes` gives each one its own hue. Natural gives the whole parenchyma one pink, so on open the five lobes are one shape. `Lobes` is one press away, and
