...
| --- | --- | --- | --- | --- |
| `the-neck-gives-way-more-than-the-inlet` — a goitre in the neck spends most of itself displacing the airway and the rest narrowing it; one that has followed the airway below the thoracic inlet, where the surroundings cannot move aside, spends most of the same tissue narrowing it. **A difference of degree, not a rule about where compression occurs** | Standard descriptions of tracheal deviation and compression in the neck, and of compression as the more prominent problem below the inlet | `deviation = push·(1 − confined)`, `indent = push·confined`, with `confined` a property of the direction and non-zero in the neck | The shares are fixed per direction, and a single distance stands for the advance | `physiology: displacement dominates in the neck and narrowing dominates at the inlet` |
