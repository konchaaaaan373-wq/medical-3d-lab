...
| `intervention-response-sizes` — **illustrative** | No source; chosen so the contrast between the two interventions is legible | `INTERVENTION_PROFILES` | Not doses, not combinable, never a response a person would have | `calibration: the cardiac-output interventions keep their chosen illustrative sizes` |
