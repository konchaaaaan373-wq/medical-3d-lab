...
| `laplace-favours-the-widest` — `T = P·r`, so at one pressure the widest part of the *distended* bowel carries the most. **Which segment that is belongs to the scenario, not to a rule**: the model solves for it, and it comes out at the caecum in the colonic closed-loop arrangements and nowhere in particular in a small bowel obstruction, where the caecum is below the blockage and the distended gut is one calibre throughout | Laplace's law for a cylinder, and the standard description of the caecum as the segment that distends most in closed-loop large bowel obstruction with a competent ileocaecal valve | `tension = areaRatio · radiusRatio · r₀(seg)/r₀(caecum)` | A thin-walled cylinder, one pressure throughout the connected length | `physiology: at one pressure the widest distended part carries the most wall tension` |
