...
and because the first version of this section **overstated it**.
...
What was available was a web search returning bibliographic records and
...
| --- | --- |
| `body-passage` | The passage this claim rests on was read in the body of the paper. **Not used by this dossier.** |
...
**Whether the source supports the claim is a separate field.** A paper can be
confirmed to exist and to say something adjacent to what an entry claims from
it, so each entry that records verification also records `claimSupport`:
...
| Entry | What the source says | What this model claims from it |
| --- | --- | --- |
| `letters-and-objects-dissociate` | Gaillard 2006, PubMed abstract (read by a reviewer, not here): reading changed after resection of a word-responsive region, with **recognition** of other visual categories preserved | A process specific to letter form can be affected while object processing is not. **Recognition is not oral object naming**, which is the task this model carries, and the entry says so rather than reading one for the other |
| `two-writing-routes` / `the-writing-route-localisation-is-not-settled` | Roeltgen & Heilman 1984, publisher SUMMARY (read by a reviewer, not here): four lexical-agraphia cases with posterior angular lesions, four phonological-agraphia cases with lesions of the supramarginal gyrus **or the insula deep to it** | The model places the two processes on the angular and supramarginal gyri. The source's phonological group is wider than that, and the entry is `uncertain` with the wider condition written out |

Consequences that follow and are not hidden anywhere else:

- Where a row says a study "found" something, it is a summary's summary.
- Sample sizes, phases (acute or chronic), imaging modality and task details
  are reported only where that summary stated them.
...
| `spatial-attention-is-not-in-the-language-hemisphere` | Standard clinical descriptions (`textbook-account`, `source-states-it`) | Attention to the left has one route through the non-dominant parietal; attention to the right has two | That the non-dominant hemisphere attends to both halves of space | `physiology: spatial attention is not in the language hemisphere` |
| `a-new-memory-needs-one-medial-temporal-lobe` | Standard accounts of the amnesic syndrome (`textbook-account`, `source-states-it`) | The memory nodes are `paired`: integrity is the better side | That one side suffices | `physiology: a new memory needs a medial temporal lobe on one side or the other` |
| `the-callosum-carries-the-other-hand` | Standard descriptions of callosal disconnection (`textbook-account`, `source-states-it`) | The left hand's route crosses the callosum; the right hand's does not | That praxis formulas are in the dominant hemisphere only | `physiology: the callosum carries the left hand, so cutting it spares the right` |
| `frontal-subcortical-circuits-share-a-signature` | Alexander, DeLong and Strick, and the clinical literature on caudate and thalamic infarcts (`textbook-account`, `source-states-it`) | Three closed loops, cortex → striatum → pallidum → mediodorsal thalamus → the same cortex | That the loops are functionally segregated where they pass through shared structures | `physiology: a frontal–subcortical circuit reads the same wherever it is cut` |
...
**Recomputed again after AR2, and nothing in this table moved.** Run from
`838ef44` over the nine modelled language tasks, all eleven presets come back
exactly as the "After" column says, and none of them reports `indeterminate`.
That is expected rather than reassuring: AR2-01 changed what happens when an
eligible route cannot be evaluated, and in atlas-lesion mode every route of
every preset here **is** evaluable — the conceptual steps on the writing routes
are `assumedAvailable`, which is a stated assumption and not an unknown. The
case AR2-01 fixes is reachable from `resolveTaskResult` directly and is pinned
there (`AR2-T01`–`AR2-T03`); it is not reachable from these eleven.

Two things about this table are worth saying plainly rather than leaving to be
inferred:

- **`dominant-inferior-frontal` still reports writing in the top band**, and it
  has not been pushed down to match the name the preset used to carry. The
  agraphia of that picture is real and this model does not produce it, which is
  declared as a coverage limitation on the result rather than corrected with a
  number nobody could defend.
- **`dominant-thalamus` reports the top band on everything, with `(＋)` on the
  five language tasks the cortico-thalamic network names.** That mark now comes
  from the damage map, so selecting the nucleus by hand says the same thing as
  choosing the preset — which it did not before AR2-03.
...
| APH-14 | Functional routes conflated with real tracts | `FUNCTION_EDGES` | **Fixed in AR2.** Every connection declares `mapping`, and the renderer draws three line types from it: solid for a tract mesh, long-dashed for a coarse stand-in, dotted for a connection with no structure. Not colour-only |
| APH-15 | Result, legend, dossier and release state inconsistent | across | **Fixed for the model, the read-out, the 3D and these documents.** What is left is the shared chrome measured in F-201 |

### AR2 — the second audit

| ID | What it said | Disposition |
| --- | --- | --- |
| AR2-01 | An unknown eligible route beside a value ≥ 0.85 was settled and the known value returned as the maximum; rounding to four places before a comparison with zero; `resolveTaskResult` classified impossible inputs | **Fixed.** Only an evaluated 1 settles the maximum; `availabilityBounds` and `establishedBand` carry what the arithmetic does settle; the model keeps exact values and `roundForDisplay` rounds at the edge; `declaredBlock` rests on a `blocked` flag set by an input, never by a product; NaN/∞/out-of-range are refused |
| AR2-02 | A positive 0.2 route was animated as a full stop with no answer, and so were `indeterminate` and `not_modeled` | **Fixed.** `routeDisplay()` is one place with five states; a positive route reaches the end with a faint answer; only a true zero halts it; the read-out writes down which state is on screen; the traced route and the task's conclusion are separate rows |
| AR2-03 | Uncomputed influence came from the preset, so the same thalamus entered as raw structures carried no warning | **Fixed.** `MODULATORY_NETWORKS` is the single declaration and is matched against the normalised damage map; a preset carrying its own is refused; `extent=0` and `severity=0` report nothing as occurring; `functionsOfStructure` returns the uncomputed involvement and applies route eligibility |
| AR2-04 | Display anchors used as causal dependencies; a missing `mapping` computed anyway; callosal part names over a whole-commissure mesh; invalid structure ids accepted | **Fixed.** `semantic-to-initiation` and `semantic-to-orthographic-lexicon` are conceptual with `displayAnchor` for drawing only; `semantic-to-phonological` declares `coarse`; `validateNetwork()` refuses an incomplete declaration at load; the callosal connections name the mesh and keep the part name in `conceptualRegion`; an unknown structure is an input error, a valid-but-unmodelled one is `outOfScopeStructures`, and with no atlas list to check against the model says it could not check |
| AR2-05 | The writing localisation was stronger than its source | **Fixed as far as a source can be read here.** Both nodes are `coarse` with `omits`; the phonological node records that the source's group included the insula deep to the supramarginal gyrus; the registry entry is `uncertain` / `via-review-material` / `source-is-narrower`. The functional dissociation is unchanged and all three conceptual experiments remain operable |
| AR2-06 | The reference layer had no way in; only the first coverage limitation was shown; 26 flat rows | **Fixed.** `ReferenceLibraryPanel` opens from the UI with keyboard, Escape and focus restore, and is a sheet on a phone; every limitation is behind a real control, with a browser check that reaches the last item; the read-out is four sections, and the rows that state limits survive the narrow-screen rule |
| AR2-07 | Three kinds of connection drawn the same way | **Fixed.** Three line types in the geometry, three legend rows, consistent across still, animation, highlight and reel. Not checked in greyscale or for colour vision — `docs/follow-ups.md` F-198 |
