...
| `one-mesh-per-named-structure` | A lesion takes whole named structures | **Approximation of the substrate.** The atlas's corpus callosum has no splenium and its precentral gyrus has no somatotopy, so those lesions are drawn coarser than they are |
...
- **Executive function, behaviour and social cognition**, deliberately: this
  model localises by route, and those are not localised that way.
