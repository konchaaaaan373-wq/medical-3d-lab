...
- **修復スクリプトは version 管理下**に置きます。同じ入力から同じ出力が出ることが、
  記録した hash の意味です
- **削除した面積ゼロ三角形と重複面の数を記録**します
  （面積ゼロ：心臓 820・大血管 26、重複面：心臓 4・大血管 5）。
  面積ゼロの三角形は何も描かないので、削除しても見た目は変わりません——
  **これが「形を作り直していない」と言える唯一の根拠**なので、数を残します
- **三角形数は前後の両方を記録**します（`sourceCounts` / `derivedCounts`）。
  削除しているのだから一致はしません。記録が測るのは「差が削除した数ちょうどか」であって、
  「変わっていないか」ではありません。`heldUnchanged.vertexPositions` も**座標だけ**を指します——
  index は三角形を削った分だけ動きます
