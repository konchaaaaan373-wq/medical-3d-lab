...
## 2026-09-24 — 配信する 2 本を Draco 圧縮する（所有者の判断）

**2026-09-15 の採用は「形は一切変えていない（no geometry was reshaped）」を前提に
していました。** 血管ファイルで Draco ではなく枝の削除を選んだのも、その一文を守るためです
（manifest の旧 `budget.targetDevices`）。ただしそのときは「量子化すると主張が崩れる」と
**論じただけで、崩れる大きさは測っていません。**

2026-09-24、「3D モデルを開くのに時間がかかる」への対応で、心臓は 6.9 MB を無圧縮で
送っていることが最大の待ち時間だと測れました（F-210）。所有者に数字を示したうえで、
**圧縮版を配信ファイルにする**と決まりました。

| | 圧縮前 | 圧縮後 |
| --- | --- | --- |
| `VH_M_Heart.glb` | 4 071 496 B | 424 932 B |
| `VH_M_Blood_Vasculature.glb` | 2 838 396 B | 434 364 B |
| 頂点の最大移動 | — | 5.1 µm / 12.2 µm |
| 法線の差（p99） | — | 0.19° |
| 閉じた面の数 | 9 / 14、10 / 37 | 同じ |
| 容積（0.1 mL 表示） | 左心室 121.6 mL ほか | すべて同じ（最大差 0.008 mL） |
| 描画（6 視点 × 2 配色） | — | 1 フレーム 60〜160 px（全 921 600 px 中）の差。描画器自体のゆらぎ 920 px 未満 |

- [x] **圧縮版を採用** — 「形は一切変えていない」は、**配信するファイルについては
  もう真ではありません。** 代わりに上の上限を主張します。ATTRIBUTION.md・
  manifest の attribution・model card を同じ日に書き換えました
- [x] **再現性** — `npm run assets:repair` → `npm run assets:compress:verify` で、
  固定したソースから同じ hash が 2 回出ることを確認
- [x] **合否の基準は script の中** — 位置 50 µm、法線 p99 1°、閉じた面の変化なし、
  容積の表示桁の変化なし。量子化を 8 bit / 7 bit に落とすと、それぞれ赤になることを確認
- [ ] **「軽い版 → 本番版」の段階読み込み** — 今回は採りません（圧縮版と本番版の見た目が
  同じなので、2 段にすると総量が増え、差し替えの一瞬が見えるだけ）。
  将来、目に見えて粗い軽量版に意味が出たときに検討します（`docs/follow-ups.md` F-211）
