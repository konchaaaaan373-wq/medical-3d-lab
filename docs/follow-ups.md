...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-159〜F-168** | 高次脳機能シーン（`higher-brain-function`）の追加。使うのは F-159〜F-164 | 2026-09-20 |
...
### F-161 高次脳機能から遂行機能・行動・社会的認知が抜けている — P2（2026-09-20）

`higher-brain-function` は言語・行為・空間性注意・記憶・Gerstmann 群を扱い、
**遂行機能と社会的行動を意図的に外しています**（model card §3）。日本の
「高次脳機能障害」の診断基準は注意・記憶・遂行機能・社会的行動障害を中核に
置くので、**名前が約束する範囲と中身がずれています**。

- **決めていないこと**: (a) 経路モデルのままでは局在を述べられないので入れない、
  (b) 別の表現（ネットワーク単位の関与度など）で入れる、(c) シーン名を
  「局在が述べられる高次脳機能」に寄せる、のどれか
- **今どうなっているか**: model card と scope panel が「入れていない、なぜなら
  経路では局在しないから」と明示しています
