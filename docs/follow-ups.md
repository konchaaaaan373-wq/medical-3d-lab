...
### F-40 溝の注釈は、アンカーが溝の底にあるため隠れる — P2（B3）

F-37 で遮蔽されたラベルを隠すようにした結果、既定の 2 つの注釈のうち
**「中心溝」が左外側ビューで出なくなりました**。アンカーが mesh の bounding box
中心＝溝の底（脳の内部）にあり、実際に手前の回に隠れているためです。
隠すこと自体は正しい——修正前のドットは中心溝ではなく**中心前回の表面**に
乗っていました（[`screenshots/f37/before/left-lateral--colour-map.png`](screenshots/f37/before/left-lateral--colour-map.png)）。
足りないのは**アンカーの取り方**で、溝のように大部分が埋もれている構造には
「その視点から見えている面上の点」が要ります。

- どう決めるか: アンカーを bbox 中心ではなく、構造の頂点のうちカメラに最も近い
  可視点にする（視点ごとに動くので、ちらつかない更新頻度も併せて決める）
- 触る先: `BrainAnatomyScene._updateAnnotationAnchors` と
  `annotationAnchors`。**変更すると model card revision が動き、
  公開判断の取り直しが要ります**
- 完了の定義: 左外側ビューで中心溝のドットが**中心溝の見えている線上**に乗り、
  右外側ビューでは出ない
