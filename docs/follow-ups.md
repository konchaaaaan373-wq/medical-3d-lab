...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は F-108 です
  （F-101〜F-106 は Landing hero の PR #72 が確保済み。この項目は当初 F-101 で
  書いていましたが、同じ番号を 2 本のブランチが同時に取る——このファイルが
  上で警告している——状態になるため F-107 に採番し直しました）
...
### F-107 シェル動線の統一で残った 1 点 — P2（2026-09-14）

モデル画面からトップへ戻る／別のモデルへ移る動線を整理しました。
`src/app/shellDestinations.js` がシェルの行き先と名前を 1 か所で持ち、
Landing / Explorer / Trust / Legal / 未公開ページ / 3D 起動失敗フォールバック /
シーンヘッダのナビゲーションがそこから生成されます
（`tests/shell-navigation.test.js`、`tests/scene-navigation-shell.test.js`）。

**Deploy Preview #73 のレビューで決まったこと**（2026-09-14）。当初ここに
「未決」として並べていた 3 点のうち 2 点は決着しました。

- **Explorer の呼び名は `Anatomy models / 解剖モデル` に確定。** 一度は
  「モデル一覧」にしていました——Lab でも β 終了後でも正しい語だからです。
  ただし未公開ページでは、すぐ下の「公開中のモデルを見る」と
  「いま見られるモデル」と並んで**一般名が 3 つ**になり、3 つの行き先に
  見えていました。β が解剖しか公開しない以上、具体名のほうが正確で、
  かつページ自身の CTA と競合しません。**β が終わって Explorer が病態も
  持つようになったら、この 1 行を変えるのが宿題です。**
- **未公開ページのヘッダと本文 CTA は、役割を分けて両方残す。** ヘッダは
  シェルのナビ、本文は 1 本の主 CTA（`openModelDestination()` が公開実体から
  生成するので、公開モデルが 1 件なら `#/organs` を経由せずそのモデルへ直行し、
  2 件目が公開された日に自動で一覧へ戻ります）と「ホームへ」だけ。
  「いま見られるモデル」欄は削除しました。

**残っている未決は 1 点です。**

- **公開モデルが 2 件以上になったとき（`heart-anatomy` 公開時）に、シーンの
  ドロワーが「モデルを選ぶもの」として読めるか。** 公開 1 件の今は
  ドロワーの主な中身が「ほかのページ」の棚で、モデル行は 1 本しかありません。
  実機でもエミュレータでも、2 件目が無いと確かめられません。
- 完了の定義: `heart-anatomy` 公開のタイミングで、ドロワーを開いた状態を
  実機で見て、モデル行と棚の優先順位を確定し本項を Resolved へ。
...
- Trust は `.trust-nav-links` を 720px 以下で `display: none` にしていたため、
  足したはずのホームが**スマホでは見えていません**でした。
- Explorer のヘッダ操作列は絶対配置で、リンクが 3 本になった結果 320px で
  「モデル情報」と「実験モデル」が**重なって**いました。
- `.locked-nav` に `flex-wrap` が無く、640px 以下の「リンクは独立した行へ」が
  効いていませんでした。

**Deploy Preview #73 のレビューで見つかったもの**（同日）。

- **未公開ページが「公開しているのは脳と心臓」と書いていました。** 公開実体は
  脳だけです。`Explorer.js` は同じ文を `PUBLIC_MANIFEST.organs` から生成して
  いて、その旨のコメントまで持っていたのに、未公開ページだけが手書きの
  文字列を持っていました。文そのものを削除して解決しています——公開モデルを
  名指しするのは manifest から生成されるボタン 1 つだけになり、ずれる文が
  存在しません（`tests/beta-release.test.js` が、説明段落が未公開の臓器名を
  含まないことを固定します）。
- **文言が内部説明・AI 文調でした。**「ジオメトリの出典とライセンスを記録し、
  実際に配信しているファイルに結びつけた公開判断が揃ってから公開します。
  画面に出るものはすべて主張だからです」——正しいが、宛先が読者ではなく
  私たちでした。2 文に置き換え、「TO BE UPDATED — 準備中」も「開発中」に。
- **`sceneFailureGuidance.js` の本文が「公開モデル一覧」と呼んでいました。**
  すぐ下のボタンとは別の名前です。本文から行き先の名前を落としました。
