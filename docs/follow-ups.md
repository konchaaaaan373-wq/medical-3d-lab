...
### F-34 部位ツリーの入口が group だけ — P2（B2-1 の残り）

初期表示では最上位 group を開くだけなので、**leaf が 1 件も見えません**。
「何があるか」を見に来た読者が、まず 2 回開かないと部位名に到達しません。

- どう決めるか: 初期展開を 2 階層にする／最初の枝だけ leaf まで開く／
  検索を付ける（検索は今回の対象外）。**実機で読んで決める**
- 完了の定義: `#/brain-anatomy` を開いた直後に部位名が見えている

### F-35 スマホの選択概要が hover 前提の文言 — P3（B1 と重なる可能性）

未選択時の案内が「触れて確認・クリック／タップで選択」で、タッチ端末に
hover はありません。文言だけの問題で、B1 の公開文言と揃えるのが自然です。
