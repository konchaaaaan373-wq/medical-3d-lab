...
### F-109 眼と耳の解剖シーンは、まだ公開の水準に無い — P2（未公開 / B3 から外した）

B3 は眼・耳・皮膚の 3 件で始めましたが、**実レンダリングを見て皮膚 1 件に
しました。** 眼と耳はゲート（`verify:anatomy`）を通っていますが、
**ゲートは絵を見ません。**

| シーン | 絵で見えたこと |
| --- | --- |
| `eye-anatomy` | 開いた直後が**ほぼ白い球**です。角膜と前房が両面 2 層ずつ重なって、読者が眼と認識する虹彩と瞳孔（`#0b080c`）を淡い灰青に洗い流しています。「眼底」視点には**直筋 3 本が横切っています**——強膜の外側にあるので幾何としては正しく、絵としては眼底ではありません。視神経乳頭と黄斑は網膜から**浮いた小球**で、面として読めません |
| `ear-anatomy` | 蝸牛が**コイルばね**です。2.5 回転の管が隙間を空けて巻いており、中を前庭蝸牛神経の太い円柱が貫いて、切り口の円盤が宙に出ています。「耳小骨の連鎖」視点で**3 つの骨が判別できません**。耳介は輪郭だけの板で、耳輪・対耳輪・耳珠がありません |

- 完了の定義: **絵で読めること**。眼は「白い球ではない開始状態」「直筋を外した眼底」
  「面として読める乳頭と黄斑」、耳は「中身の詰まった蝸牛」「見分けのつく 3 つの骨」
- 測り方: `npm run shots:anatomy -- --dist <preview> --scene <slug> --preview`。
  証跡は [`screenshots/pub-b3/held/`](screenshots/pub-b3/held/)
- **どちらもゲートは通ります。**「候補一覧にある」ことと「見た」ことが別だという
  B1・B2 と同じ話で、3 バッチ続けて同じことが起きています
