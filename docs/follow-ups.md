...
### F-191 スマホでは充満圧が読み出しの折り返しの下に落ちる — P2（心拍出量）

外部レビューの要求「CO の増加だけを『改善』に見せない。充満量を増やすときは
LVEDP や肺静脈側の圧を一緒に見せる」は、**デスクトップでは満たしています**
——1440×900 で CO・SV・MAP・左室拡張末期圧・平均肺静脈圧の 5 行が同時に見えます
（`shots` で確認済み）。

**390×844 では満たしていません。** 読み出しパネルは MAP までで切れ、
左室拡張末期圧と平均肺静脈圧は折り返しの下です。パネルはスクロールしません。

これは F-188（スマホで 3D がパネルの下に潜る）と同じ共通 framing の問題で、
このシーンだけの修正で閉じるものではありません。候補は 3 つ:

1. 読み出しをスマホでスクロール可能にする
2. スマホでは行数を絞る（どの行を残すかはシーンが決める）
3. framing 側で上端の帯を確保し、パネルの高さを与える（F-188 と同じ修正）

**どれもこのシーンの外に影響するので、公開の変更とは別の 1 本にします。**
