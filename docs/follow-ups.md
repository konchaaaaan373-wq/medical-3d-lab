...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は F-128 です（F-111〜F-123 は 09-15 に main 側で採番済み）。
  ⚠ **F-124 / F-125 は 2 回ずつ出ていました。** 未マージだった PR #73 と PR #75 が
  先に確保しており、この branch も同じ番号を使っていました。**この branch 側を
  F-124〜F-127 に採番し直しています**（旧 F-124→F-124、F-125→F-125、
  F-126→F-126、F-127→F-127）。main 側の F-124 / F-125 はそのままです
...
F-126 で臨床レビュー登録簿（98.5 kB）は外しましたが、まだ eager なのは：
...
- 確かめ方: `node scripts/review-states.js` と同じ要領で eager graph を測る
  （F-126 の PR に使った手順が `scripts/review-states.js` の冒頭にあります）。
...
### F-119 プレビュー build が見せないもの — P3（2026-09-15）

`npm run verify:gated` は**エンタイトルメントを持つ読者**として paid surface を
開きます。**持たない読者が見る面は、そのぶん見えなくなります**——
購入導線、`feature-lock` の状態、「まだ契約していません」の文言。
これらも面であり、誰も測っていません。

素のビルドがその状態そのものなので、見ること自体は簡単です。
必要なのは**チェックを書くかどうか**の判断だけです。

- **どう確かめるか**: `verify:gated` に `--locked` を足し、`?preview=1` なしで
  同じシーンを開いて lock 状態の面を測る
- **完了の定義**: lock 状態の面についても「使用の限界を述べる文が 12px 以上」が
  実測されている
...
| ~~`elbow-anatomy`~~ | ~~「recolouring left 0 rows marked selected」1 件のみ~~ | **解決（2026-09-14 / B3）。製品ではなく検査の側でした**——下の F-124 を参照。いまは 4 点が別々の構造に当たります（上腕骨・上腕二頭筋腱・上腕三頭筋腱・関節包） |
...
- **残りは `hand-anatomy` だけです。** 3 件とも `verify:anatomy` は通りますが、
  **手が通るのは「外した点を測り直す」からです**（F-124）。記録済み 4 点のうち
...
### F-124 ブラウザ検査が、自分の狙いのずれを製品の欠陥として報告していた — 解決（2026-09-14 / B3）
...
### F-125 安全域フィットが遠近を無視していた——奥行きのある被写体が枠から出る — 解決（2026-09-14 / B3）
...
### F-126 眼と耳の解剖シーンは、まだ公開の水準に無い — 一部解決（2026-09-15） / P2（未公開）
...
### F-127 開いた直後の構図と、「表示をリセット」後の構図が違った — 解決（2026-09-15）
...
- **F-126 eager entry の gzip が予算の手前** — 解決（2026-09-15）。**89.7 kB → 65.9 kB**
...
- **F-125 Trust ページの tab 巡回が閉じない** — 解決（2026-09-15）。
...
- F-124 — 2026-09-14 — ブラウザ検査が、開始状態で測ったクリック座標をカメラが
...
- F-127 — 2026-09-15 — 読み込み直後の構図が、シェルがまだ印を付け終えていない
...
- F-104（`elbow-anatomy` と `lymphatic-drainage`） — 2026-09-14 — 肘は検査側の
  欠陥（F-124）で、直したら 4 点が別々の構造に当たるようになりました。リンパ系は
  記録済みの点がそのまま通ります。**`hand-anatomy` は開いたままです**——通るのは
  再照準のおかげで、構図そのものは直っていません。

- F-125 — 2026-09-14 — `fitPoseToSafeArea` が透視カメラに対して正射影の計算を
