...
`npm run verify:hero-touch`（`scripts/check-hero-touch.mjs`）が Chromium の
...
`final-browser-validation.yml` の `hero-touch` ジョブが回します。
...
- **残っているのは物理端末です。** エミュレーションは同じイベント経路を
  デスクトップのハードウェアで通しているだけで、指の接触面積・実際の
  `touch-action` の合成・ブラウザのタップ遅延・発熱は見えません。
  とくに iOS Safari は別エンジンで、ここでは一度も動かしていません。
- 確かめ方: iOS Safari と Android Chrome でトップページを開き、脳をタップ／
  ドラッグ／往復ドラッグ／ピンチする。親指でカードが隠れないか。
- 完了の定義: 2 端末で記録し、問題があれば hero 側で閉じる。

### F-103 タッチでは「ピンチで拡大」がモデルではなくページに効く — P2（2026-09-14）

hero の操作ヒントは「ドラッグで回転・ピンチ／+−で拡大」と書いていますが、
`landingOrganViewport.js` は canvas に `touch-action: pan-y pinch-zoom` を
設定していて、これは**縦スワイプとピンチをページに返すための意図的な指定**です。
実測（上記のタッチドライブ）でもピンチは `visualViewport.scale` を動かしており、
モデルのカメラではなくページが拡大します。つまりヒントの文言が実際より
多くを約束しています。

- 決めること: (a) 文言をタッチとポインタで出し分ける（タッチでは
  「ドラッグで回転」だけにする）、(b) canvas のピンチをモデルに割り当てて
  ページズームを諦める、(c) 現状を許容して文言だけ直す。
  **(b) は hero がページの一部である以上おすすめしません**——記事の途中で
  ピンチが効かない領域ができます。
- 完了の定義: どれかに決めて、文言を変えるなら `tests/landing.test.js` で固定する。

### F-105 hero の部位選択はポインタ専用 — P2（2026-09-14）

hero の部位名カードはクリック／タップでしか出せません。キーボードでは
矢印キーで回転・+− で拡大・Home で初期視点に戻せますが、**部位を選ぶ手段が
ありません**。固定した名前は live region で読み上げますが、そこへ到達する
経路がポインタしかない、ということです。カード自体は `aria-hidden` で、
読み上げは live region が担当しています。

本体モデル（`#/brain-anatomy`）にはキーボードで歩ける部品ツリーと検索が
あるので、**機能が無いのではなく hero に無い**状態です。

- 決めること: hero に最小の選択手段を置くか（例: Tab で「部位を選ぶ」に入り、
  ↑↓ で主要部位を巡回）、hero は概観に徹して本体モデルへ誘導するか。
  後者なら、その導線自体は F-102 と同じ話になります。
- 完了の定義: どちらかに決め、前者なら `tests/landing.test.js` に
  キーボード経路を固定し、`npm run verify:ui` の Tab 走査に載せる。

### F-104 ブラウザ検証スクリプトの静的サーバが 9 本に複製されている — P3（2026-09-14）

`dist/` を配る同じブロック（MIME 表・traversal を止めた `fileFor`・
`createServer`）が `check-viewports.mjs` / `check-anatomy-interaction.mjs` /
`check-patient-explanation.mjs` / `capture-anatomy-views.mjs` /
`check-heart-recipe-report.mjs` / `check-disease-interaction.mjs` /
`check-auth-flow.mjs` / `b5-journey.mjs` にそれぞれ写しで入っています。
今回 `scripts/lib/serve-dist.mjs` を作って `check-hero-touch.mjs` だけが
使っています。**残りは移していません**——1 本ずつ実ブラウザで回して確かめる
必要があり、この変更の範囲を超えるからです。

- 完了の定義: 各スクリプトを `serveDist()` に載せ替え、それぞれの
  `npm run verify:*` を 1 回ずつ通す。移すたびに MIME の差分
  （`.ktx2` などを足したものがないか）を確認する。

### F-102 部位名カードから本体モデルへ繋ぐか — P3（2026-09-14）

hero のカードは名前と階層だけを出し、説明文（`description` / `note`）は
出していません。hero は入口であって読み物ではない、という判断です。

- 決めること: 選択した部位を引き継いで `#/brain-anatomy` を開く導線
  （例: 「この部位を詳しく見る」）を足すか、hero は名前だけで止めるか。
  `BrainAnatomyScene.selectStructure()` は id を取るので、ルート側に
  構造 id を渡す口を作れば繋がります。
- 完了の定義: どちらかに決めて、足すなら `tests/landing.test.js` に導線を固定する。
