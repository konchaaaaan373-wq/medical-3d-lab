...
- **2026-09-12 の変更点**: 再設定フォームは `<form>` になり、Enter で送信できます
  （それまでは `type="button"` だったので Enter が効きませんでした）。
  受信箱が要る往復そのものは未実施のままです。
...
確認メールが有効な場合、**再送の導線が無い**ことも併せて決める必要があります
（メールが迷子になった人は、同じアドレスで登録し直すしかありません）。

- 確かめ方: 本番かプレビューで、使っていないアドレスで `新規登録` を 1 回通す。
  その場でログイン済みになるか、確認メールの通知が出るかを見る。
- 完了の定義: どちらであるかがこの台帳に記録され、確認メールが有効なら
  再送導線を出すか出さないかを決める。
