...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は F-116 です（F-108〜F-110 は 2026-09-14〜15、F-111〜F-115 は
...
### F-115 390px のシーンに医学的注意書きが 1 つも出ていない — P1（2026-09-15）

**公開中の唯一のシーン（`brain-anatomy`）を 390px で開くと、
「教育用」「臨床使用不可」「診断」「学習用」のいずれも画面に出ません。**
768px と 1280px では出ます。

原因は `.disclaimer` が狭い幅で `display: none` になっていることです
（`src/styles/` のどこかの media query。**main でも同じ**で、
F-113 の作業で入った変更ではありません——`origin/main` のビルドで確認済み）。

実測（Chromium、production ビルド）:

| 幅 | `.disclaimer` | 画面上の注意書き |
| ---: | --- | --- |
| 390px | `display: none` | **なし** |
| 768px | 表示 | 「教育用」「臨床使用不可」 |
| 1280px | 表示 | 「教育用」「臨床使用不可」 |

**β で公開しているのは解剖モデル 1 件だけで、その 1 件がスマホでは
何の但し書きも持ちません。** 隠したのは場所の都合だと思われますが、
「狭いから出さない」は医学的な境界の記述に対する扱いとしては説明になりません。

- **決めること**: スマホでどこに置くか。(a) 常時表示の 1 行に短縮
  （`disclaimer-short` が既にある）、(b) タイトルカード内に畳む、
  (c) 初回のみのシートで出して以後は情報パネルに置く
- **完了の定義**: 320〜430px で、シーンを開いた直後に注意書きが
  読める状態で画面にあることを実機で確認した記録がある
