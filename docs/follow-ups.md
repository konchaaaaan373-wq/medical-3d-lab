...
### F-212 — 心拍出量の実験レイアウト: 実機受け入れ・初見の理解・3D の読みやすさ — P2

2026-09-25（3 回目の実機確認を受けて）: **ページは 1 つの実験で開きます**——
「心臓の収縮を弱めると、どう変わる？」「収縮を弱める」「元に戻す」。押すと収縮力（Ees）だけが
2.74 → 1.2（収縮力低下プリセットと同じ値）になり、読み出しの表には SV・CO と、逆向きに
動く左室充満圧が出ます。「ほかの条件を試す」で血管抵抗↑・充満量↑の実験へ（基準から開始）。
4 つの入力・開始状態・介入は「詳しく調整」の中で、いまの条件を引き継ぎます。
比較・データ・レッスンは「その他」へ。比較は条件が開始時と違うときだけ押せ、2 つの心臓は
視線に直交する向きに並べます（world x に並べると片方がカメラに近く、同じ条件でも大きさが違って見えた）。

`verify:disease` は、最初の 1 押しで動くのが 1 入力だけであること、画面（ボタン・読み出し・
console）が 1 px も動かないこと、カメラが動かないこと、表示が解の値であること、「詳しく調整」が
同じ条件を引き継ぐこと、「元に戻す」で開始に戻ること、比較が開始時には押せないことを強制します。

**残っていること**

1. **実機での受け入れ確認**（iPhone Safari・Android Chrome）。確認済みは headless Chromium の
   エミュレーションだけ
2. **初見の人が説明なしで理解できるか**は、自動テストでは測れません。「開く → 収縮を弱める →
   変化を見る → 元に戻す」を初見の人に操作してもらう確認が要ります
3. **3D の読みやすさ**: 内腔が見える向き・コントラストは変えていません。収縮を弱めたときの
   動きの変化が 3D で読めるかは目視が要ります
4. 比較の「変更前」の心臓は意図して半透明の別の見た目です（どちらが前かを区別するため）。
   それが「形が違う」と読まれないかは実機で確認が要ります
5. 電話で「詳しく調整」を開くと実験の欄は隠れ、モデルは小さくなります（390×664 で約 114 px）
