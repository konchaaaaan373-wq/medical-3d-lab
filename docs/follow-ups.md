...
### F-212 — 心拍出量の実験レイアウト: 実機受け入れ・電話の心臓の大きさ・3D の読みやすさ — P2

2026-09-25 の再設計（所有者の実機録画 12:08 / 14:09 を受けて）: **4 つの入力
（充満量・血管抵抗・収縮力・心拍数）を常に 1 行で名指しし、選んだ 1 つを位置の変わらない
エディターで動かす**（スライダー＋「減らす／増やす」ボタン、開始時の目盛り、
「この項目を戻す」「全体を戻す」）。開始状態と介入は「開始状態・介入」の中。
介入のあとで 1 つ動かしても、他の入力は介入の値のまま（「ドブタミン（模式）適用後を調整」）。
読み出しは最初から「開始時 → 現在 ±差」で、差の欄が後から出て操作を押し下げることはない。
矢印は方向だけ（↑↑ の大きさ区分は廃止）。変更前の内腔の線は廃止（比較は「変更前と並べる」）。

`verify:disease` が 8 サイズで、最初の入力・ドラッグ・続けての入力でエディターが 1 px 以上
動かないこと、ドラッグが途切れずページがスクロールしないこと、44 px の当たり、入力の選択で
値が変わらないこと、1 つ戻すと他が残ること、ドブタミン後の手動調整で他の入力が残ること、
「全体を戻す」がその実験の開始に戻ること、どの状態でもモデルが覆われないことを強制します。

**残っていること**

1. **実機での受け入れ確認**（iPhone Safari・Android Chrome・タブレット・横向き）。
   確認済みは headless Chromium のエミュレーションだけ（1440×900・1280×720・1024×768・
   430×932・390×844・390×664・375×667・375×553）。実機ではとくに: ドラッグ中に縦スクロールや
   3D 回転へ入力が流れないか、Safari のツールバー伸縮で console が動かないか
2. **390×664 で、操作中のモデル（心臓＋ループ）は高さ約 114 px**（以前の「スライダーを開くと
   約 80 px」の状態は無くなった）。「開始状態・介入」を開いている間は約 59 px。
   ループを含めて枠に収めているので、心臓そのものはその 6〜7 割です。さらに大きくするには、
   電話で心臓だけを枠に入れる（ループは画面の端で切れてよい）等の判断が要ります。375×553 は報告のみ
3. **3D の読みやすさ**: 内腔が見える向き・コントラストは今回変えていません。覆われないことは
   測っていますが、「変化が読めるか」は実機で目視が要ります
4. **比較の起点の定義**: 起点は常に「この実験を開始した状態」。基準と収縮力低下を並べて比べる
   （開始状態どうしの差）は出しません。要るなら別の設計判断
5. `circulation` は既定のレイアウトのまま
