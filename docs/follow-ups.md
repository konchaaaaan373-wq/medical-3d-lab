...
- **いま床を下回っているものを記録**し、**その一覧は伸びることを許さない**。
  新しい小さい字は、それを足した PR で落ちます
- **床を超えたら一覧から消す**ことを要求します。消し忘れも落ちるので、
  一覧は自然に短くなります
- `npm run type-floor` で確認、`npm run type-floor -- --write` で短くする

**この PR で 215 → 213 になりました。** 上げたのは、`.disclaimer`
（「⚠︎ 教育用肉眼解剖 — 臨床使用不可」、10.5px → 12px）と
`.landing-demo-boundary`（11px → 12px）——**モデルが何でないかを述べる文**が、
実寸で描かれたモデルの下でいちばん小さい字だったためです。

**次に減らす候補**（医学的な境界の記述が床を下回っているもの）:

| セレクタ | いま | 面 |
| --- | ---: | --- |
| `.patient-guide-boundary, .patient-guide-educational` | **9px** | 患者説明（課金ゲート） |
| `.education-guide-boundary` | 9.5px | 教材（課金ゲート） |
| `.patient-guide-certainty` | 10.5px | 患者説明（課金ゲート） |
| `.clinical-review-badge`（title-card） | 9px | シーン |
| `.consent-detail` | 11.5px | 同意 |

**課金ゲートの内側は認証セッションが無いと実機で見られません。**
上げる前に見る手段を決める必要があります（`?preview=1` ビルド + stub 認証）。
