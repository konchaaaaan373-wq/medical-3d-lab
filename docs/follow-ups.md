...
- 確かめ方: `.github/workflows/final-browser-validation.yml` は engine を matrix で
  回せます。`verify:disease` をそこに足し、Firefox / WebKit の結果を本項に書く。
...
### F-163 16:9 プリセットのカードがモデルに重なる（画面・ファイルの両方） — P3（リール）

心不全を 16:9 で再生すると、EF のカードが心臓の上半分に重なります。
**これは書き出し側の不具合ではありません**——同じ瞬間の画面を撮って並べたところ、
DOM のオーバーレイも同じ位置に同じ大きさで重なっており、ファイルは画面に忠実です
（2026-09-21、`heart-failure` を 16:9 で録画 → 復号したフレームと、
同じ時刻の画面写真を比較）。**プリセットそのものの性質**で、`reel.css` の
16:9 上書き（`.reel-cards { top: 6% }`・`.reel-card-ef { font-size: 6em }`）が、
この距離のカメラでは主題の上に落ちます。

- **直し方を「書き出し側だけで」やらないこと。** 合成が画面と違うレイアウトを
  持った瞬間、「ファイルは読者が見たものと同じ」という性質を失います（L-55）。
  直すなら `reel.css` と painter の表を**同時に**動かします（テストが両者を
  突き合わせているので、片方だけ動かすと赤くなります）。
- 決めること: 16:9 でカードを下げるか、カメラを引くか、16:9 ではカードを出さないか。
  9:16 と 4:5 は重なりません（`shots` で確認済み）。
- 完了の定義: 16:9 で主題とカードが重ならない状態を実レンダリングで確認し、
  その値が CSS と painter の両方に入っている。
