...
### F-129 肺: hero の初期姿勢では Enter が何も選べない — P1（2026-09-16）

`npm run verify:hero-input` が実測で報告（production ビルド）:

```
keyboard (desktop, lungs): Enter named "色分けされた部位をクリックすると、
  解剖学的な名称が表示されます。"   ← ヒント文字列＝何も選ばれていない
  ...after turning Enter named "左上葉 左肺 › 肺葉 › 葉"
```

hero の Enter は「画面中央にあるもの」を尋ねます。肺の初期姿勢では
**中央が左右の肺のあいだの隙間**に落ちるので、**最初の Enter は何も選びません**。
回してから押すと正しく動きます。

**これは F-121 と同じ「Enter が黙って何もしない」不具合**で、しかも
訪問者が最初に触る面です。肺はこの 1 件のために公開から外しました
（シーンページ自体の `verify:anatomy` は 4 点 4 種類で緑）。

- 肺は名前つき tour を持ちました（右上葉・右中葉・左上葉・左主気管支）。
  tour は正しく、hero の狙いが正しくありません
- 直し方の候補: (a) 肺の hero 初期姿勢を、中央が実体に載る向きにする
  (b) hero の照準を「中央」ではなく「モデル上の最寄り点」にする
  ——(b) は正中に隙間のある臓器すべてに効きます（肺・腎・左右対の臓器）
- **この検査が無ければ公開していました。** #102 で hero drive を
  公開臓器ごとに回すようにした効果がそのまま出ています
- 完了の定義: `verify:hero-input` が肺でも「turn する前の Enter」で構造名を返し、
  そのうえで公開記録を取り直すこと
...
### F-128 公開できる臓器は hero builder のある 5 つに限られる — P1（2026-09-16）

knee は名前つき tour を持ち（6 点が 6 種類を指す、調べた 6 件で最良）、
drive も通ります。**それでも公開しませんでした。**

landing の既存テスト 2 本が合わさって次を要求しているからです:

- chooser のコントロール数 ＝ `PUBLIC_MANIFEST.count`（公開臓器 1 つに 1 つ）
- hero rotation の各 entry は `hasOrganModel(organ)` を満たすこと

つまり **公開 ⟹ hero entry ⟹ hero builder** です。
`ORGAN_HERO_BUILDERS` にあるのは **brain / heart / lungs / liver / kidney の 5 つだけ**。
knee を公開すると「chooser が描けない公開臓器」ができます。

- **F-122 の費用にこれを足してください。** 1 件あたり「名前つき tour + 記録」に加えて、
  hero builder が無い臓器には**軽量な hero モデルの作成**が要ります
- 現状、hero builder がありゲートも通る臓器は kidney だけですが、
  その kidney は F-124 で止まっています。**つまり今の仕組みのままでは、
  次に公開できる臓器はありません**
- やること: どちらを先にするか決める——(a) hero builder を増やす、
  (b) 「公開臓器は必ず hero に出る」という不変条件自体を見直す
- 完了の定義: 上のどちらかが決まり、次の 1 件が公開できる状態になっていること
...
**24 件中 9 件が「人だけ」です**（L-01 / L-05 / L-06 / L-09 / L-13 / L-14 /
L-15 / L-16 / L-23）。**L-04 は解決しました**——`scripts/lib/css.mjs`。
...
- どう確かめるか: `npm run lessons` の「9 of 24」が下がること。
