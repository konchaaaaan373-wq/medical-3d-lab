...
### F-110 開いた直後の構図と、「表示をリセット」後の構図が違う — P2（公開済み 10 件に影響）

B3 で検査の狙いを直しているときに出ました。**同じシーン・同じ視点なのに、
読み込み直後とリセット後でモデルの大きさと位置が違います。**
リセット後の方が**明らかに大きく**、読み込み直後は小さく上に寄っています。

- 証跡: [`screenshots/pub-b3/framing-at-load/`](screenshots/pub-b3/framing-at-load/)
  の `nose-anatomy-load.png` と `nose-anatomy-after-reset.png`。
  1 か所クリックして「表示をリセット」を押しただけで、あいだに
  カメラ操作はありません
- **読者が最初に見るのは小さい方です。** 公開済みの 10 件を含め、
  `shots:anatomy` の証跡もすべて読み込み直後の構図なので、
  **これまで見てきた絵は読者が見る絵と同じ**です。つまり承認した内容は
  変わりませんが、**枠をもっと使える**ということです
- 分かっていないこと: 何が違うのか。候補は 2 つで、どちらも未確認です——
  (a) 読み込み時の `setShot` が canvas の最終サイズより前に走っていて、
  `viewer.camera.aspect` と `safeAreaInsets()` が確定していない。
  (b) 部位パネル（`.rail`）の高さが選択の有無で変わり、`railAcrossTop`
  （「上端の帯か、右の帯か」）の判定が反転する
- 測り方: 読み込み直後と、1 クリック＋「表示をリセット」後の 2 枚を撮って
  重ねる。原因を当てる前に、**`safeAreaInsets()` と `camera.aspect` を
  両方の時点で読み出す**こと
- 完了の定義: 同じ視点なら、いつ押しても同じ構図になること。
  直したら**公開済み 10 件を撮り直して見る**（構図が変わるため）
