...
- 決めるべきは 2 択: **(a)** 大血管を別の出典付き asset で足す（その asset にも
  同じ inventory・ライセンス確認が要ります）／**(b)** 収録範囲を限定した scope として
  作り、UI・model card・名称で「何が入っていて何が入っていないか」を明示する
- 次の実装ステップ（どちらを選んでも共通）: asset manifest への登録、`public/` への
  配置、adapter（ontology id → 構造 id）、scene、共通 viewer への接続。
  **公開判断は別**で、大血管の扱いが決まるまで開けません
