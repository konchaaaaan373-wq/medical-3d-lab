...
### F-212 — 心拍出量の「実験装置」レイアウト: 実機受け入れと、状態比較の baseline 定義 — P2

`cardiac-output` を `meta.layout = 'experiment'` に移しました（状態 → 介入 → 心臓 →
数値の順、4 本のスライダーは「詳細パラメータ」の奥、補助ツールは「その他」の奥、
読み出しは「操作前 → いま ＋Δ」）。PR 内で直したもの: 1024〜1280px で詳細パラメータが
心臓に重なる（左の列を framing の帯として予約）、手動調整中も「介入なし」が点く
（「手動調整」の状態表示）、「学ぶ」が波形パネルに重なる（左の列を console の上で止め、
lesson 中は ① ② を隠す）。

確認済み: headless Chromium の **1440×900・1280×720・1024×768・390×844** で、
心臓と panel / console / lesson の重なり、手動調整の表示、reset 後の表示を実測。
`verify:disease` 緑。

**残っていること**

1. **実機での受け入れ確認** — 2026-09-25 に iPhone（Safari、production）の画面収録を
   受け取り、**心臓がパネルの裏に 2 倍で写る**（L-114）・**数字が心臓に被る**という指摘を
   受けました。直したもの: fit の最小帯を下げて小さくても覆われない位置に置く、電話では
   タイトル・読み出し（3 列の帯）・心臓・操作を縦に積む、言語／パネルを隠す／ご意見を
   「その他」へ、平均肺静脈圧を「すべての数値」へ、画面の目的を 1 行でタイトルの下に。
   `verify:disease` が 6 サイズで「どのパネルもモデルを覆わない」を強制します。
   **まだ見ていないもの: 修正後の実機**、Android、タブレット、横向き
2. **電話の心臓は小さい**（390×664 で高さ約 100 px、375×553 で約 50 px）。覆わないことを
   優先した結果です。375×553 では目的の 1 行と各数値の「変更前」を隠しても帯が数十 px で、
   `verify:disease` は報告のみ。大きくするなら、電話で読み出しを畳む（例: 1 行の要約だけ
   残す）か、心臓とパネルを切り替える設計が要ります
3. **状態比較の baseline 定義**（別 issue 候補）。状態（基準心 ↔ 収縮力低下）を切り替えても
   Δ は出しません。選んだ状態がそのまま比較の基準になるためで、「基準心 → 収縮力低下で
   何が変わるか」を Δ で見せるなら、何を baseline と呼ぶかは教材設計とモデルの意味の
   判断です。曖昧なまま Δ を出すより、現在値だけを示すほうが安全として保留
4. 1280×720 で reader が自分で orbit したあとに「学ぶ」を開くと、lesson の上端と心臓の
   外接箱が 4px 触れます（reader が持っているカメラは動かさない、という既存の規則の結果）
5. `circulation` も同じ形（状態を 1 つ選ぶ）ですが、既定のレイアウトのままです
