...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-200〜F-209 / L-98〜L-107** | ナビゲーションと App Shell の UX 監査（reading surface 間の document 再読込み廃止・共通ヘッダ・Trust の IA・boot veil・モデル切替の frame handover）。使うのは F-200〜F-208 と L-98〜L-103。**採番し直し 1 回目**: F-197〜F-206 / L-94〜L-103 で取りました。マージの直前に main を読み直したところ、**F-197・F-198・F-199 と L-94〜L-97 が全部すでに使われて**いて（#151 が同じ F-197〜F-206 / L-95〜L-104 を予約していました）、**main の最大（F-199 / L-97）の上へ丸ごと移しました**。F-198 は `### F-198` ではなく解決済みの箇条書きとして書かれているので、**見出しだけを数えると空いて見えます**——予約は着手時に読んだ最大ではなく**マージ時点の最大**から取り直し、見出し以外の使用も数えること | 2026-09-23 |
...
### F-189 公開した `cardiac-output` に臨床レビューが無い — P1（公開中）

**2026-09-22、所有者の判断で公開しました。臨床家は誰もこのモデルを
読んでいません。** 登録簿は `pending`、画面には `アルファ` と
`医学レビュー：未完了` が出ており、公開判断記録
（[`beta-publication/cardiac-output.md`](beta-publication/cardiac-output.md)）と
[ADR](architecture/adr-2026-09-22-mechanism-scene-in-beta.md) が、それを
自分の言葉で述べています。**隠してはいませんが、解消してもいません。**

レビューを受けるべき対象は、少なくとも次の 4 つです。

1. **基準条件**（`REFERENCE_GEOMETRY` と較正値）——教科書の代表値に着地するよう
   選んだもので、測ったものではありません
2. **`収縮力低下` プリセットの大きさ**——説明用に選んだ値です
3. **介入 2 件の大きさ**——循環充満 +120 mL、ドブタミンの ×1.5 / ×0.85。
   出典の効果量からは導いていません（F-183）
4. **操作域の端が教材として妥当か**（F-180）

**これは公開を止める理由には「しない」と決めた項目です**（所有者判断）。
止める理由になるのは、**患者向けの面を足すとき**——`patient-explanation` を
宣言する、または患者ビューを付けると、ゲートが現行レビューを要求して閉じます。
`tests/cardiac-output-scene.test.js` がその 1 行を固定しています。
...
**誤りになる**ことです。7 区画モデルに変えられる静脈容量がないので、
「抵抗だけを上げる薬」に縮約するほかありません。ところが設計書が挙げた一次文献
