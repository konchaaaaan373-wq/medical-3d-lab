...
`docs/verification-lessons.md` が 32 件を持ち、`npm run lessons`（`npm test`
からも走る）が台帳の側を守ります——3 つの欄が**あって中身が空でない**こと、
**名指ししたガードが実在すること**、まだ人しか捕まえられない件数。

**32 件中 12 件が「人だけ」です**——`npm run lessons` の出力から書いています。
L-01 / L-05 / L-06 / L-09 / L-13 / L-14 / L-15 / L-16 / L-24 / L-26 / L-28 / L-31。
（L-32 はガードを持つので、ここには入りません。最初 13 と書いたのは、
出力を読まずに「足したから増えた」と決めた私の誤りです。）
**L-04 は解決しました**——`scripts/lib/css.mjs`。
（L-21 は並行 branch と番号が衝突し、後から採番したこちらが L-31 へ移りました。）
...
- **F-111 モデルが 2 つ公開されたあとのヘッダーの出口** — 解決（2026-09-17）。
  ...
  **測定そのものが 3 回嘘をつきました**（L-31）。閉じた `<details>` の中身は
