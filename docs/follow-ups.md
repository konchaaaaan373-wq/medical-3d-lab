...
**原因を測りました（2026-09-16）。z-index ではありません。**
失敗の瞬間に幾何を読むと:

```
sheet box   [0, 489, 375,  38]   ← 高さ 38px
body box    [14, 660, 347,   0]  ← 高さ 0、しかもシートの外（下）
row box     [14, 644, 347,  32]  ← シートの外。innerHeight 667 も超える
elementFromPoint(行の中心) → CANVAS
sheet: fixed/40   panel: static/auto   stage: fixed/auto
```

**シートは 38px に潰れ、パネル本体は高さ 0 で、シートの外（y=660）にいます。**
一覧の行はそこからさらに溢れて y=644..676——画面の下端（667）をまたいで、
**stage の上に置かれています**。z-index は正しく（シート 40 / stage auto）、
行がシートの内側にいないので効いていないだけです。

だから「stage が行の上に来る」という書き方も不正確でした。正しくは
**行がシートの中にいない**。

- **未解決の問い**: 膝・肩・肝・肺は同じ電話の節を通っています。
  なぜ腎だけ本体が高さ 0 になるのか、**まだ分かりません**。
  見えている行が 2 つしかないこと（枝が畳まれている）と関係がありそうですが、
  **確かめていません**
- やること: 上の問いを先に測る（他シーンで body box を同じ瞬間に読めば
  1 回で分かります）。そのうえで直す
