...
**壁厚の判定は genus に置き換えました**（閉じた多様体・1 成分のときだけ）。中実の棒は球＝genus 0、
壁のある管は輪環＝genus 1。結果：

- **開いた面 26 本**：genus は定義できず、**判定不能**
- **非多様体/複数成分 2 本**（左冠動脈・上大静脈）：mesh が壊れていて判定以前
- **閉・多様体・genus 2〜6 が 8 本**（肺動脈幹・左右肺動脈・肺静脈 4 本・左右腕頭静脈）：
  貫通孔は**ある**が、壁・分岐ループ・自己接触・欠陥のどれかは**特定できない**
- **閉・多様体・genus 0 が 1 本**（小心臓静脈）：貫通孔なし＝**この 1 本は壁厚なし**

つまり**確定したのは 1 本だけ、残り 36 本は未確定**です。画面の説明も「確認中」のままにしています。
...
**強くなった主張もあります**：左室は閉・多様体・1 成分・**genus 0** で 121.60 mL。
貫通孔が無い＝環状の壁ではなく中実の体積、かつ心腔サイズ。内部ビューを出さない根拠は
ray の数ではなくこれです。左房 genus 4、右室 genus 26 は、肉柱のある鋳型として記録のみ。
