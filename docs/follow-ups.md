...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は **F-130** です（F-111〜F-123 は 09-15、F-124 は 09-15、
  F-125 は 09-16 に main 側で採番済み。F-126〜F-129 はこの branch）。
  ⚠ **F-107〜F-110 の二重採番は解消しました。** main 側と PR #73 / PR #75 が
  同じ 4 つを同時に確保していたので、**この branch 側を F-126〜F-129 に
  動かしています**（旧 F-107→F-126、F-108→F-127、F-109→F-128、F-110→F-129）。
  main 側の F-107〜F-110 と、そこを指している本文はそのままです
...
コンテンツはチェック側が `entitledGuide()` を呼んで答えます。見えなくなるものは F-126。
...
| ~~`elbow-anatomy`~~ | ~~「recolouring left 0 rows marked selected」1 件のみ~~ | **解決（2026-09-14 / B3）。製品ではなく検査の側でした**——下の F-126 を参照。いまは 4 点が別々の構造に当たります（上腕骨・上腕二頭筋腱・上腕三頭筋腱・関節包） |
...
- **残りは `hand-anatomy` だけです。** 3 件とも `verify:anatomy` は通りますが、
  **手が通るのは「外した点を測り直す」からです**（F-126）。記録済み 4 点のうち
...
### F-126 ブラウザ検査が、自分の狙いのずれを製品の欠陥として報告していた — 解決（2026-09-14 / B3）
...
### F-127 安全域フィットが遠近を無視していた——奥行きのある被写体が枠から出る — 解決（2026-09-14 / B3）
...
### F-128 眼と耳の解剖シーンは、まだ公開の水準に無い — 一部解決（2026-09-15） / P2（未公開）
...
### F-129 開いた直後の構図と、「表示をリセット」後の構図が違った — 解決（2026-09-15）
...
- **F-126 プレビュー build が見せないもの** — 解決（2026-09-15）。
...
- F-126 — 2026-09-14 — ブラウザ検査が、開始状態で測ったクリック座標をカメラが
...
- F-129 — 2026-09-15 — 読み込み直後の構図が、シェルがまだ印を付け終えていない
...
- F-104（`elbow-anatomy` と `lymphatic-drainage`） — 2026-09-14 — 肘は検査側の
  欠陥（F-126）で、直したら 4 点が別々の構造に当たるようになりました。リンパ系は
  記録済みの点がそのまま通ります。**`hand-anatomy` は開いたままです**——通るのは
  再照準のおかげで、構図そのものは直っていません。

- F-127 — 2026-09-14 — `fitPoseToSafeArea` が透視カメラに対して正射影の計算を
