...
### F-204 3D の線種は、15 秒シーケンスの画角では見分けられない — P3（2026-09-22）

3 種類はジオメトリで分かれており（頂点アルファ 0 の隙間）、テストが
存在と**世界座標での破線長がほぼ一定であること**を固定しています。
絵で測った結果:

| 画角 | 見分けられるか |
| --- | --- |
| 1440×900・既定 | **見分けられます。** 太さの差（0.026 / 0.018 / 0.012）が効き、破線の隙間も見えます |
| 1440×900・4 段階ズーム | はっきり見えます |
| 15 秒シーケンス（9:16、脳の幅 ≒ 360px） | **見分けられません。** 経路全体が 120px ほどで、破線 1 周期が 6px 前後になります |

今回の改修:

- 破線の長さを**区間の割合ではなく世界座標で固定**しました
  （`DASH_RING = 0.035`）。割合だと短い連絡は短い破線、長い連絡は長い破線に
  なり、同じ線種が 2 種類に見えます
- 太さの差を広げました（実線 0.026 / 長い破線 0.018 / 点線 0.012）。
  **太さは画角に依存しない手がかり**です
- **凡例の見本を線にしました**——実線・破線・点線を、3D と同じ太さ順で。
  文章だけの凡例は「読まないと使えない凡例」です

- **残っていること**: reel の画角では依然として読めません。その画角で
  読めるほど破線を大きくすると、既定の画角では「点の列」になります。
  reel の overlay は線維束の同定を主張していない（注記が
  「示しているのは経路の通りやすさ」と言っている）ので、**P3** に置きます
- **まだ見ていないこと**: 白黒印刷と色覚特性。線種は色に依存していませんが、
  確かめたのは画面 1 種類です
