...
### F-31 in-scene rail が満杯 — 部位ツリーと選択カードが同時に見えない — P1（B2-1）

部位ツリーを rail に足した結果、desktop 1280×800 でも rail の中身
（inspection / 選択カード / 部位ツリー / legend / ボタン）が入りきりません。
ツリーをスクロールして部位を選ぶと、**その名前を出しているカードが上に流れて
見えなくなります**。ID の一致は保たれていますが（3D・ツリー・カードは同じ
`bx_id` を参照）、答えが画面外にあるのでは読めません。

- **試して却下した案**: 選択カードの `position: sticky`。ツリーの行の上に
  カードが被り、`verify:ui` が occlusion として検出しました
  （まさにあの検査が拾うべき失敗です）。ツリー側の二重スクローラも同様に、
  行がクリップ境界を跨いで「見えるが押せない」状態を作りました
- どう決めるか: rail の情報設計そのもの。ツリーを inspection のような
  開閉サーフェスに移す／カードとツリーを 1 枚にまとめる／rail を 2 段にする、
  のいずれか。**320〜1280 px で実測してから決める**
- 高さ 520px 未満ではツリーを非表示にしています（landscape phone）。
  これも同じ問題の応急処置で、恒久的な答えではありません
- 完了の定義: 部位を選んだとき、その名前が同時に見えている。
  `verify:ui` が 6 viewport で緑、`verify:anatomy` が緑
