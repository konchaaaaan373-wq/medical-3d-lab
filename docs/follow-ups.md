...
### F-101 シェル動線の統一後に残した判断 — P2（2026-09-14）

モデル画面からトップへ戻る／別のモデルへ移る動線を整理しました。
`src/app/shellDestinations.js` がシェルの行き先と名前を 1 か所で持ち、
Landing / Explorer / Trust / Legal / 未公開ページ / シーンヘッダの
ナビゲーションがそこから生成されます（`tests/shell-navigation.test.js`、
`tests/scene-navigation-shell.test.js`）。残っているのは次の 3 点です。

- **Explorer の呼び名を `All models / モデル一覧` に統一しました。** それまでは
  Landing が「解剖モデル」、Trust と Legal が「モデル」、ドロワーが
  「モデル一覧」「公開モデル」と 4 通りでした。β が解剖だけである以上
  「解剖モデル」の方が今は正確ですが、Lab と β 終了後は誤りになるため
  スコープに依存しない語を採りました。**β の間だけ「解剖モデル」に戻すかは
  未決です。** 決めるなら `SHELL_DESTINATIONS` の 1 行です。
- **シーンのドロワーは公開モデルが 1 件でも出したままにしました。**
  以前は `hasChoices` が false のとき trigger ごと隠れており、公開 β
  （公開モデル 1 件）のモデル画面にはナビゲーションが 1 つも無い状態でした。
  いまは 1 件でもドロワーが開き、その中に「ほかのページ」の行き先が入ります。
  **公開モデルが 2 件以上になったとき（`heart-anatomy` 公開時）に、この
  ドロワーが「モデルを選ぶ」ものとして読めるかは実機で見ていません。**
- **未公開ページ（`LockedSurface`）のヘッダにナビゲーション行を足しました。**
  本文の CTA（「公開中のモデルを見る」「ホーム」）と重複します。ヘッダはナビ、
  本文は文としての CTA という住み分けで残していますが、片方に寄せるかは未決。

**監査（2026-09-14、同日）で見つけて直したもの** — 記録として残します。
最初の実装は「行き先を 1 か所で持つ」ところまでで、**それが画面に届いているか**を
測っていませんでした。4 件とも実測で確認してから直しています。

- `SceneFailureFallback` だけ移行し忘れていました。3D が起動できなかった読者が
  出る面、つまり出口が最も要る場所が、最後まで「公開モデルを見る」「実験室」と
  独自の名前を使っていました。
- Trust は `.trust-nav-links` を 720px 以下で `display: none` にしていたため、
  足したはずのホームが**スマホでは見えていません**でした。ブランド以外に出口が
  ない状態です。行をブランドの下に回すようにしました。
- Explorer のヘッダ操作列は絶対配置で、リンクが 3 本になった結果 320px で
  「モデル情報」と「実験モデル」が**重なって**いました。狭い幅では絶対配置を
  やめてヘッダの 1 行目にしています（DOM 順も先頭に移したので、視覚順と
  フォーカス順が一致します）。
- `.locked-nav` に `flex-wrap` が無く、640px 以下の「リンクは独立した行へ」が
  効いていませんでした。

- 完了の定義: 上の未決 3 点を実機（iOS Safari / Android Chrome）で確認し、
  呼び名とドロワーの扱いを確定して本項を Resolved へ。
