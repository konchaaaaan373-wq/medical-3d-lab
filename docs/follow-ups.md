...
- ~~血管の面が内腔か壁かを測っていない~~ → **半分は測れました**。
  `npm run assets:measure`。各 mesh の重心から 128 方向へ ray を飛ばし交差回数を数えると、
  37 血管すべてで最頻値は 2（1 枚の面を出入り）か 0/1（開口端から抜ける）で、
  **4（外壁→内壁→内壁→外壁）になるものは 1 つもありません**。
  つまり**壁厚はモデル化されていない 1 枚の面**です。
  ただし**その面が内腔をなぞるのか外表面なのかは形状からは決まりません**——
  口径比較は個体差の範囲に入り、出典に segmentation の意図の記載もありません。
  この未確定は画面の説明にもそのまま書いています
