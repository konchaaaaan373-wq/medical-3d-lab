...
| --- | --- | --- | --- |
| knee-anatomy | **0.16** | 0.50 | **9 / 10** |
| shoulder-anatomy | **0.22** | 0.24 | **8 / 10** |
| liver-anatomy（公開） | **0.28** | 0.42 | 6 / 10 |
| lung-anatomy（公開） | **0.32** | 0.50 | 5 / 10 |

**「2.6 倍細い」は消えました。** 膝と肝の比は 1.75 倍、肩と肝は 1.27 倍です。
そして幅と逆に**高さは関節のほうが使っています**——膝は 9 行中 8 行、肩は 7 行に
載るのに対し、肝は 5 行、肺は 4 行。

幅 × 行数で粗く見ると膝 0.16×9/10 = 0.144、肝 0.28×6/10 = 0.168 で、
...
**上の表は計器を直したあと測り直した値です（2026-09-16、2 回目）。**
初出はシルエットの掃引が `0.45` の行を見ておらず、再掃引の窓も狭く、
右端の列も落としていました（L-30）。**幅は 4 件とも変わりませんでした**——
変わったのは掃いた行が 9 → 10 になったことによる分母だけです。
壊れた計器で出した数をそのまま残さないために測り直しており、
「変わらなかった」もその測定の結果です。
