...
- 確かめ方: 本番かプレビューで、使っていないアドレスで `新規登録` を 1 回通す。
  その場でログイン済みになるか、確認メールの通知が出るかを見る。
- 完了の定義: どちらであるかがこの台帳に記録される。挙動の実装はもう分岐しません。

### F-99 `npm run budget` が eager entry で超過している — P2（既存）

`npm run budget` が **`entry (eager JS) 134.7 kB / 90 kB`** で落ちます。
CI（`ci.yml`）は `npm run budget` を実行するので、**この枝は今日すでに赤**です。

**ログイン動線の作業で増えたものではありません。** 分岐元（`8e170c7`）を
worktree で別に build して測ったところ、`134.7 kB / 90 kB` と**数値まで同一**でした。
同じ比較で css は 36.7 → 36.8 kB、code は 523 → 524.3 kB で、どちらも予算内です。

超過は 45 kB ぶんで、原因の切り分けも縮小も performance 側の独立した作業なので、
認証の変更には混ぜていません（混ぜれば、どちらのレビューも成立しなくなります）。

- 確かめ方: `npm run build && npm run budget`。何が eager に入っているかは
  `scripts/check-bundle-budget.js` が出す内訳と `dist/assets` の chunk を突き合わせる。
- 完了の定義: eager entry を 90 kB 以下に戻すか、`BUNDLE_BUDGET_KB`
  （`src/app/performanceBudget.js`）を上げて**なぜ製品が重くなってよいのかを
  PR に書く**か、どちらかを選ぶ。放置して赤のままにしない。
