...
1. **実機での受け入れ確認**（特にスマートフォンのタッチ操作と実 GPU）。Deploy Preview
   公開後に production と同じ 1 本の URL で見る。headless の 390×844 は emulation であって
   保証ではありません。software GL では orbit の慣性が秒単位で残るため、「介入の前後で
   同じ視点」も**ドラッグしていない状態**でしか測れていません
2. **状態比較の baseline 定義**（別 issue 候補）。状態（基準心 ↔ 収縮力低下）を切り替えても
...
3. 1280×720 で reader が自分で orbit したあとに「学ぶ」を開くと、lesson の上端と心臓の
   外接箱が 4px 触れます（reader が持っているカメラは動かさない、という既存の規則の結果）
4. `circulation` も同じ形（状態を 1 つ選ぶ）ですが、既定のレイアウトのままです
