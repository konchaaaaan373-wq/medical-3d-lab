...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-177〜F-186** | 失語モデルの拡張と reel の実レンダリング（使うのは F-177 のみ。**採番し直し**: F-167 で取ったところ、上の行が先に取っていました——L-16 の形をもう一度踏みました） | 2026-09-21 |
...
- **重複の一覧は [`tests/follow-ups-numbers.test.js`](../tests/follow-ups-numbers.test.js) が持ちます。ここには書き写しません。**
  ...
  **13 回目（この PR）** 上の「後から来た側」が今度は先に居た側になりました——
  失語モデルを広げる branch が F-167 / L-55 / L-56 を取り、rebase したら
  12 回目の採番し直しが**同じ番号に着地していました**。F-177〜F-186 と
  L-60 / L-61 へ動かしています。**shallow clone を fetch せずに採番した**のが
  両方の原因で、これで 2 回続けて同じ理由です。
...
### F-177 15 秒シーケンスの実レンダリング — 見た。残りは 2 点 — P3（2026-09-21）
...
（`docs/verification-lessons.md` L-60）、(2) 「どこで止まったか」が
