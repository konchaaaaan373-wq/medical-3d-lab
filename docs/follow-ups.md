...
| `elbow-anatomy` | 「recolouring left 0 rows marked selected」1 件のみ | 4 点は当たります。配色を変えたあとツリーの選択行印が消える、という 1 点だけが残ります。**製品側の可能性があります**（選択は保持されるのに、ツリーの marked 表示だけ落ちる） |
...
- 完了の定義: 3 件とも `npm run verify:anatomy` が通ること。ただし `hand-anatomy` は
