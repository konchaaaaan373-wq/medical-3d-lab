...
### F-101 Landing hero の部位名カードをタッチ端末で確かめる — P2（2026-09-14）

トップページの hero で、クリック／ホバーした部位の解剖学的名称を
モデルの上に出すようにしました（`src/app/landingOrganHero.js` の
`.landing-demo-structure`、`src/app/landingOrganViewport.js` の
`onStructureChange`）。名前は `BrainAnatomyScene` が返すものをそのまま出しており、
名称そのものはアトラス由来で、この変更は新しい医学的主張をしていません。

確認済みなのは Chromium（headless）での 320 / 390 / 768 / 1280 px です
——ホバーで仮表示・クリックで固定・固定がホバーに勝つ・空所クリックで解除・
カードがドラッグを奪わない（`pointer-events: none`）・横スクロールなし。

- **未確認はタッチ端末です。** タッチにはホバーがないので、実機では
  「タップ＝固定」だけが経路になります。回転ドラッグの終わりが 7px 以内に
  収まったときに選択と読まれないか、カードが親指の下に来ないか、
  `touch-action: pan-y pinch-zoom` のままタップが拾われるかは実機でしか見えません。
- 確かめ方: iOS Safari と Android Chrome でトップページを開き、脳をタップ／
  ドラッグ／ピンチする。カード（画面下）が操作の邪魔にならないこと。
- 完了の定義: 2 端末で記録し、問題があれば Landing の CSS か
  `OrganAnatomyScene` 側のドラッグ閾値ではなく hero 側で閉じる。
