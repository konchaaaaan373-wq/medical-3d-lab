...
### F-204 3D の線種は、拡大しないと見分けられない — P3（2026-09-22）

実線・長い破線・点線は**ジオメトリ**で分かれており（頂点アルファ 0 の隙間）、
テストが 3 種類すべての存在を固定しています。**絵で確かめたところ**、
1440×900 で 4 段階ズームすると破線ははっきり見えますが、
**既定の画角と 15 秒シーケンスの画角では見分けられません**——線が細く、
破線の周期が画素より小さくなります。

- **なぜ問題か**: 「実トラクトと概念連絡を見分けられる」ことが AR2-07 の
  必須条件です。凡例には 3 行あり、読み取り欄も経路の種類を言いますが、
  **絵だけで分かるのは拡大したときだけ**です
- **やるなら**: 破線の周期を線の太さではなく**画面上の長さ**で決める
  （カメラ距離に応じて `on`/`off` のリング数を変える）。
  あるいは既定のズームを少し寄せる
- **まだ見ていないこと**: 白黒印刷と色覚特性での見え方。線種は色に
  依存していませんが、確かめたのは画面 1 種類です（F-198 と同じ項目）
