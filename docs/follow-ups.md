...
- ok 4G／5G で脳モデルが読み込まれる
- ok 3D モデルが表示される
- ok 回転できる
- ok 拡大・縮小できる
- ok 部位を選択し、名称と詳細カードが表示される

**静止画では判定できないので、以下は pass にしていません**——タップ 1 回で
選択されたか、往復スワイプで誤選択が起きないか、縦スワイプ／ピンチを 3D 領域が
奪っていないか、VoiceOver が選択部位を 1 回だけ読むか、横向き表示、
親指での押しやすさ。**これらは未確認のまま**です。
