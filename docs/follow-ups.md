...
### F-41 心臓解剖（X3）は素材の入手で止まっている — P1（B4 / X3）

X3 の最初の関門は「候補素材を**実際に読んで** inventory を出す」ことです
（pack 05 §2、拡張指示 §5.2）。この実行環境では**そこで止まります**。名前や
形式からの推測で先へ進めないため、以下を記録して停止しました。

**試したこと（2026-09-08、この環境）**

| 候補 | 結果 |
|---|---|
| NIH 3D HRA Heart, Male `3DPX-021000` | `https://3d.nih.gov/` へ CONNECT が **403**（環境の network policy が当該ホストを許可していない）。ページも実ファイルも読めていません |
| NIH 3D HRA Heart, Female `3DPX-020966` | 同上 |
| GitHub 上の代替候補の探索 | このセッションの repository scope は `konchaaaaan373-wq/medical-3d-lab` に限定されており、**他リポジトリの横断検索は禁止**されています。よって候補の発見自体を行っていません |

到達可能なのは `raw.githubusercontent.com` / `api.github.com`（HTTP 200 を確認）
だけで、既知の URL があれば取得できますが、**ライセンスを名前から推測しない**
という規則があるため、URL の当て推量では進めません。

**やっていないこと（意図的）**

- 既存の procedural な心臓ビルダーを GLB に書き出して「出典のある形状」に
  昇格させること（拡張指示 §5.3 が明示的に禁止）。
- 素材のない `heart-anatomy` scene の殻を作ること。§5.1 の到達点は
  「**実素材の**モデルがローカル表示で動く」ことであって、空の scene ではありません。
- 内部構造（心腔・弁・中隔・冠動脈）を推測で作ること。

**何があれば進むか（どれか 1 つ）**

1. `3d.nih.gov`（および実ファイル配信ホスト）を環境の network policy で許可する
2. 候補ファイルそのもの、または取得可能な固定 URL・commit・hash を渡す
3. 候補リポジトリを `add_repo` の対象として指定する

**その後にやること**は指示書どおりに固定されています: 固定 version/hash/取得日時、
構成物ごとのライセンス、node/mesh/material の実 inventory、四腔・壁・内腔・大血管・
弁・中隔・冠動脈が**別々に扱えるか**（無いものは missing と記録）、軸・単位・
variant、triangles と転送 bytes。**素材が外表だけなら「内部まで完成」とせず、
検証済みの外表 scope として記録します。**
