...
- **次の番号は、Resolved を含めた全体の最大値 + 1 です。** 開いている項目だけを
  見て決めると衝突します——実際 2026-09-06 に F-24 と F-25 が別々の件に
  二重採番され、片方は Resolved に移っていたため気づかれませんでした。
  そのときは**後から採った側**を F-29 / F-30 に付け替えています
  （先に採った側を参照している文書があるため）。
  「再採番しない」は 1 番号 = 1 項目を守るための約束なので、
  重複したまま残すほうが約束を破ります
...
### F-27 肺・肝の detail は「ジグザグ隠し」のまま — P2（稜線カット）

`carvePart` が稜線に沿って切るようになり、切断面はテッセレーションに依存しなく
なりました（F-24）。ところが**恩恵を回収したのは腎臓だけ**です。

- 肺 `detail = 12`、肝 `detail = 10` は、どちらもコメントに「切断面のジグザグが
  見えるから」と書いてあります。原因が消えたのに値は据え置きです
- そのうえ稜線カットは三角形を **+17%**、ビルド時間を肝で **+44%** 増やします。
  つまり両者は「もう要らない細分」と「新しい上乗せ」を二重に払っています
- 腎臓は 18 → 12 に下げて、旧 18 比で三角形 55%・ビルド 31% 減でした
- 完了の定義: 肺と肝を実描画で見ながら detail を下げ、
  `tests/lung-anatomy.test.js` / `liver-anatomy.test.js` の share が
  解像度で動かないことを確認する

### F-28 脳は A2 のまま手つかず — P2（3 臓器の作り込み）

「脳・心臓・腎臓を完璧に」で実際に手が入ったのは心臓（冠動脈の巻き方向・
大動脈基部の蓋）と腎臓（集合系・稜線）だけです。**脳に対しては
`organs/brain.js` の脳幹チューブが閉じただけ**で、A2 の本体である
`brainAnatomy` の標本アトラス（437 メッシュ）は測ってすらいません。

- アトラスは外部由来で hash 固定されているため、メッシュ品質は「直す」対象では
  なく「記録する」対象です。まず `tests/mesh-quality.js` で測って、
  閉じているか・外を向いているかを anatomy-specs に書くべきです
- 完了の定義: アトラスの品質を測って記録し、脳に対する A2 の主張が
  何に支えられているかを 1 か所に書く
...
### F-30 「Lab をモデル中心のカタログへ」（旧 PR #28）の意図 — P3
...
- **F-24 — 2026-09-07 —** 小腎杯が乳頭ごとの受け皿になり、上・中・下の 3 本の
  大腎杯を経て腎盂へ開くようになりました。切断面のジグザグは
  `carvePart` が稜線に沿ってメッシュを切ることで解消し、detail は 18 → 12。
  残っていた「腎洞を刳り抜いていない」は F-24 の完了条件ではなく、
  `anatomy-specs.md` §3 の受け入れ済み単純化として残ります。
- **F-29 — 2026-09-06 —**（採番時は F-24。番号が重複していたので付け替えました）
  WebKit 固有でも読み込み失敗でもなく、**ページ遷移で中断された
