...
**3 回目の測定（2026-09-17）: 「2 つしか指せない」は間違いでした。**
数えていたのは**押せる構造の数ではなく、4 点がどこに落ちたか**です。
シーンに直接聞くと（`structures.filter(x => !x.hidden && x.currentOpacity > 0.14)`）:

| 状態 | 押せる構造 |
| --- | --- |
| 開いた直後 | **5 / 32** — 皮質・腎全体・左右尿管・膀胱 |
| 層 0.35 | 21 / 32 |
| 層 0.5 | 20 / 32 |
| 層 0.7 | **31 / 32** |
| 層 1.0 | 15 / 32 |
| **冠状断面ビュー（層は 0 のまま）** | **32 / 32** |

つまり:

- 開始時点でも **5 つ**であって 2 つではない。4 点のうち 2 点が
  **意図的に 1 構造として置かれた右腎**に当たっていたのが「2 種類」の正体です
  （右腎はランドマーク形状で、境界の無い名前を与えないための設計）
- **内部に到達する手段はシーンが既に持っています**——層スライダー（0.7 で 31/32）と、
  **冠状断面ビュー（切り替えるだけで 32/32）**
- 層 1.0 で 15 に減るのは、皮質と髄質が ghost 側に抜けて腎杯・腎盂だけが残るためで、
  これは設計どおりです

**残る問いは 1 つだけで、それは不具合ではなく構図の判断です**——
「このシーンは何を見せて開くべきか」。現状は外側から見た左右の腎で、
到達できる名前は 5 つ。CLAUDE.md は見えにくさを camera / opacity で直すことを
明示的に認めており、オーナーの継続指示も「中を見せる」です。
選択肢は測ってあります（上表）。**勝手に構図を変えていません。**
