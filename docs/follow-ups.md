...
| ~~`elbow-anatomy`~~ | ~~「recolouring left 0 rows marked selected」1 件のみ~~ | **解決（2026-09-14 / B3）。製品ではなく検査の側でした**——下の F-107 を参照。いまは通ります |
| `lymphatic-drainage` | 同上 | 221 点中 hit 7。細い管と結節群なので、格子で当てるには疎すぎます。3 点は記録済み hit から手で置きました |
...
- 完了の定義: 残り 2 件とも `npm run verify:anatomy` が通ること。ただし `hand-anatomy` は
  **構図を直してから**——淡くて小さいモデルは、検査が当てられないだけでなく読者も当てられません
