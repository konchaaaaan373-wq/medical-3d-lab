...
### F-169 動画書き出しを確かめたブラウザは 1 つだけ — P2（動画書き出し）

`npm run verify:disease` が reel を持つ 5 シーンすべてで
「同意 → 録画 → 書き出したファイルをブラウザ自身に再生させて 1 フレームを撮る」
まで通しました（2026-09-21）。ただし**通したのは Playwright の Chromium 1 つだけ**です。

- **未確認**: Firefox と WebKit（iOS Safari を含む）。`MediaRecorder` は
  WebKit 17 以降にありますが、`canvas.captureStream()` との組み合わせと
  書き出した WebM の再生は測っていません。対応していないブラウザでは
  ボタン自体を出しません（`videoRecordingSupported()`）ので、
  **最悪でも「出ない」で、壊れた画面にはなりません**——が、
  「iOS では出ない」が正しい状態なのかは決めていません。
- **既知の性質**: `MediaRecorder` の WebM には duration がヘッダに書かれないため、
  書き出したファイルの再生時間は**プレイヤーが最後まで読むまで不定**です
  （検査は `duration not written by the recorder` と印字します）。これは
  形式の性質であり録画の失敗ではありません。直すには後処理で
  Cues/Duration を書き足す必要があり、依存を増やさずにやる方法は調べていません。
- **走らせる場所は用意しました（2026-09-21）。** `verify:disease` が
  `--engine chromium|firefox|webkit` を取るようになり、
  `.github/workflows/final-browser-validation.yml` に **`video export (engine)`**
  の 3 本 matrix を足してあります（1 エンジン 1 シーン。測っているのはエンジンで、
  シーンではありません）。**この環境では走らせられません**——Playwright の
  配布 CDN がネットワークポリシーで 403 になり、Firefox / WebKit の実行ファイルを
  取得できないためです（`cdn.playwright.dev` / `playwright.download.prss.microsoft.com`）。
- 残っているのは**その workflow を候補 commit に対して 1 回起動し、結果をここに書くこと**
  だけです（`mcp__github__actions_run_trigger`、`commit_sha` と `pr_number` が要ります）。
- 完了の定義: 3 エンジンの結果（ファイルが出るか / 再生できるか）を本項に記録し、
  出ないエンジンについては「出さない」を製品判断として 1 行で確定する。


### F-170 `verify:ui` は reel mode を開かないので、その面は測られていない — P2（動画書き出し）

視点マトリクス（`verify:ui`）が測るのは通常の面です。**シーケンス（reel mode）に入った
状態は 1 度も測っていません**——そこは `#ui.is-reel` が上部バーと console を隠し、
canvas が画面比どおりにリサイズされる**別のレイアウト**です。

- 実際に見つかったこと（2026-09-21）: 動画の保存ボタンを足したところ、
  390px で chip の行が**両端とも画面外**に出ました（`9:16` が左、`Exit (Esc)` が右）。
  `npm test` も `verify:ui` も緑のままで、見つけたのは
  `npm run shots:phone -- --scene copd --preview` の写真です。
  直したのは `.reel-chrome` の折り返しで、CSS の側は
  `tests/video-export.test.js` が規則として読んで固定します——が、
  **「はみ出していないこと」を測っているのは相変わらず誰もいません。**
- 確かめ方 / 決めること: `scripts/check-viewports.mjs` に「reel mode に入った状態」を
  1 面として足せるか（シーンごとに入退場が要るので、面の数は増えます）。
  足すなら、どのシーン 1 件で代表させるか。
- 完了の定義: reel mode の面が `verify:ui` の測定対象に入り、はみ出し・44px 床・
  重なりが他の面と同じ基準で測られる。あるいは「写真で見る」を正式な方針として
  1 行で書き、本項を Resolved へ。
...
- **床はあえて低くしてあります（2026-09-21 改訂）。** 一度 4 視すべてで ΔE 12 以上まで
  ...
  差し戻しました（[L-65](verification-lessons.md)）。いまの床は**表面 8 family で
...
### F-167 動画にできるのは「シーケンスを持つシーン」だけ — P3（動画書き出し）

動画書き出しは reel（15 秒のシーケンス）を持つシーンにだけ現れます。
**他の病態シーンはアニメーションしていても書き出せません**。
（件数はここに書きません——`tests/video-export.test.js` が
`getReel()` を持つシーンを数えて突き合わせます。実際、この branch を開いている間に
main が 6 件目を足し、**そのガードが気づきました**。）`videoExportProblems()` は
その理由を `has no animated sequence to record` と返します。

- **これは制限であって欠陥ではありません。** 書き出す 15 秒を誰かが構成していない
  シーンで「いまの画面を 15 秒」録ると、始まりも終わりも主張も無い動画になります。
  ここで足すべきは録画の対象ではなく、**シーケンス（`getReel()`）のほう**です。
- 決めること: シーケンスを持たない病態シーンに、汎用の「いまの状態を N 秒」録画を
  出すか、出さないか。出すなら、そのファイルが何を主張しているのかを
  フッターだけで言い切れるかを先に決める。
- 完了の定義: どちらかに決め、決めたほうを 1 行で書いて本項を Resolved へ。

### F-168 書き出しの解像度は canvas のバックストアで、1080×1920 ではない — P2（動画書き出し）

録画は表示中の canvas をそのまま composite するので、**解像度は画面の大きさと
devicePixelRatio で決まります**。検証環境では 506×900 でした（9:16 の形は正しい）。
SNS の推奨は 1080×1920 で、そこへ引き伸ばすのはプレイヤーの仕事になります。

- **引き伸ばしてから録るのは選びませんでした。** 小さい canvas を 1080 幅へ
  拡大すれば数字は満たせますが、中身はぼけた 506 px です。嘘の解像度より
  正直な解像度のほうがましだ、という判断です。
- 決めること: 録画中だけ renderer を目標サイズへ resize するか
  （画面のレイアウトが動くので、録画中は「準備中」の表示が要ります）、
  それとも**大きな画面で録ってください**と案内するだけにするか。
- 完了の定義: どちらかに決める。resize を選ぶなら、録画中のレイアウト移動が
  実ブラウザで見て許容できることまで含めて完了。


### F-171 16:9 プリセットのカードがモデルに重なる（画面・ファイルの両方） — **対応済み**

心不全を 16:9 で再生すると、EF のカードが心臓の上半分に重なります。
**書き出し側の不具合ではありませんでした**——同じ瞬間の画面と復号したフレームを
並べたところ、DOM のオーバーレイも同じ位置に同じ大きさで重なっており、
ファイルは画面に忠実です。プリセットそのものの性質です。

**直し方（2026-09-21）**: 動かしたのはカードでもカメラでもなく、**背景**です。
16:9 と 1:1 では比較が画面いっぱいになるので、逃がす場所がありません。
カメラを引けば、自分のキャプションのために主題が小さくなります。そこで
marker と callout が既に使っている**半透明の下敷き**を、短い画面比のときだけ
カードにも付けました（`reel.css` と `reelFramePainter.js` の両方。
色と適用する画面比は `tests/video-export.test.js` が突き合わせます）。

- **残り**: 9:16 と 4:5 は重なりません（下敷きも付きません）。
  16:9 の構図そのもの——カードを別の場所に置くか、比較を 16:9 では 1 つにするか——は
  シーンの作り手の判断で、ここでは触っていません。必要になったら
  `reel.css` と painter の表を**同時に**動かします。

### F-172 書き出しのコードは、使えない production ビルドにも載っている — P3（動画書き出し）

実測（2026-09-21、production ビルド・gzip）:

| | 直前の main（`c00d6e3`） | この branch |
| --- | --- | --- |
| entry (eager JS) | 73.5 kB | 73.5 kB（変化なし） |
| css | 41.5 kB | 42.1 kB |
| code (JS + CSS) | 593.8 kB / 700 kB | **602.4 kB / 700 kB** |

**+8.6 kB。** そして production ビルドでは**誰もこの機能に触れません**——公開して
いるのは解剖シーンだけで、解剖はシーケンスを持たず、ルールが弾くからです。
`App.js` が静的に import しているので、painter・recorder・同意ダイアログ・文言が
App chunk に入ります（リール本体が同じ理由で既に入っているのと同じ形です）。

- **いま削らない理由**: 予算に 98 kB 余っており、`CLAUDE.md`「予算・重複・整理は、
  詰まっているときだけ」に従います。**先に台帳に置くのが約束**なので、数字ごと置きます。
- 削るときの形: `recordVideo` の中で `await import('./videoRecorder.js')`、
  同意ダイアログも押されたときに読む。`videoExportOffered` はボタンを出すかを
  決めるので静的なまま（catalog を既に読んでいるので増えません）。
- 完了の定義: 遅延読み込みにしたうえで、この表を測り直して差分を書く。
  あるいは「この重さで持つ」と決めて 1 行書き、Resolved へ。
