...
### F-212 — 心拍出量の実験レイアウト: 実機受け入れ・電話の心臓の大きさ・状態比較の baseline — P2

2026-09-25 の再設計（所有者の実機確認を受けて）: **操作は 1 行のシナリオ**（基準／収縮力低下／
充満量↑／後負荷↑／ドブタミン、スライダーで外れると「カスタム」）、スライダーは「自分で調整」、
電話は「上＝心臓、その下＝数値の帯、下＝シナリオ」、デスクトップは「左＝心臓、右＝数値・
シナリオ」、スライダーはデスクトップでは左の列・電話では下の枠内でスクロール。変化の大きさは
矢印の形（↑／↑↑／≈）と言葉で示し、3D には変更前の拡張末期の内腔を淡い線で重ねる。
`verify:disease` が 6 サイズで、静止時・介入後・スライダーを開いた時のいずれでも
「どのパネルもモデルを覆わない」を強制します。

**残っていること**

1. **実機での受け入れ確認**（iPhone Safari・Android・タブレット・横向き）。確認済みなのは
   headless Chromium の 1440×900・1280×720・1024×768・430×932・390×844・390×664・375×667・375×553
2. **Safari 程度の高さ（390×664）では、心臓とループは高さ約 140 px**（スライダーを開くと約 80 px）。
   覆わないことを優先した結果です。さらに大きくするなら、電話で数値の帯を畳む／心臓と操作を
   切り替える等の設計判断が要ります。375×553 は報告のみ
3. **状態比較の baseline 定義**（別 issue 候補）。「収縮力低下」は起点として現在値だけを示し、
   基準との差は出しません
4. **介入のあとでスライダーを動かすと、介入の効果は外れて開始条件＋そのスライダーになります**
   （`ExperimentSession` の既存規則）。画面は「収縮力低下 → カスタム」と数値で正しく示しますが、
   「ドブタミンにさらに上乗せ」を期待した読者には意外かもしれません。上乗せはモデル側の判断
5. 変更前の内腔の線は、スライダーで大きく動かすと網目状に見えます。見え方の調整は P2
6. `circulation` は既定のレイアウトのまま
