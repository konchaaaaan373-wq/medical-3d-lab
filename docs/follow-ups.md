...
- 先に `npm run verify:live-auth` を実行してください（F-98）。確認メールが
  そもそも送られる設定かどうかが分かるので、**どのメールを探すのか**が決まります。
  自動確認が有効なら登録時の確認メールは存在せず、探すべきは再設定メールだけです。
- ここだけは受信箱が要るため自動化していません。**認証サービスに送信させるのは
  実在の人の受信箱への副作用**なので、検証スクリプトは GET しか行いません。

### F-98 新規登録のメール確認が有効かどうかは未確認 — P2（ログイン動線）

`signUp` の応答に session が含まれるかどうかで動線が 2 つに分かれます。含まれれば
その場でログイン済みになり、含まれなければ「確認メールを送信しました」を出して
ログイン側に戻します。**コードは両方を扱いますが、この Supabase プロジェクトで
実際にどちらが有効かは確認していません。** stub を相手にした Chromium 実測では
どちらの分岐にも入れないため（`/auth/v1/signup` が 400 を返すところまで）、
ここは実デプロイでしか分かりません。

**再送導線は 2026-09-12 に作りました。** `resendSignUpConfirmation`
（`POST /auth/v1/resend`）を、**session を伴わない signUp 応答が返ってきた直後だけ**
——つまり確認メールが有効なときだけ——フォームに出します。自己ゲートなので、
プロジェクト設定を事前に知らなくても、要る場面にだけ現れて他では出ません。
アドレスは本人が今入力したものなので、列挙の懸念もありません。

**両分岐とも `npm run verify:auth` が実測済みです**（session 無し →
確認メールの案内・ログイン側へ復帰・アドレス保持・再送が `/auth/v1/resend` に届く、
session 有り → そのままログイン済みになり再送は出ない）。
つまり「どちらが起きても正しく動く」ことは分かっており、
**未確認なのは実プロジェクトでどちらが有効かという事実だけ**です。

**確かめるのに登録は要らなくなりました（2026-09-13）。** Supabase の
`GET /auth/v1/settings` は公開の読み取り専用エンドポイントで、アドレスを
自動確認するかどうかを自分で公表しています。`npm run verify:live-auth` が
**配信中の bundle から** origin と publishable key を読み（リポジトリの値ではなく、
実際に配られているものを読むので、環境変数がずれているケースもここで出ます）、
その設定を問い合わせてどちらの分岐が生きているかを報告します。

- 確かめ方: `npm run verify:live-auth -- https://med-3d-lab.necofindjob.com`
  （ネットワークのある場所から。このリポジトリの実行環境からは proxy の
  CONNECT 403 で本番に到達できません —— F-05 と同じ制約です）
- **このスクリプトは GET しかしません。** 登録もしない・再設定メールも送らない・
  アカウントも残らない。実在の受信箱に副作用を出さないための制約です
- 完了の定義: 出力をこの台帳に 1 行記録する。挙動の実装はもう分岐しません。
