...
（報告「COPD のリンクを押したら脳のモデルになる」への対応）。
**4 経路のうち 2 つは実ブラウザで確認済み**です:

| 経路 | 確認 |
| --- | --- |
| 離脱時に前のモデルが見えない | ✅ Chromium 実測。`elementFromPoint` が中央で veil の `SPAN`、main では `CANVAS` |
| 到着（`#/copd` → locked ページ） | ✅ 実測 |
| skip link（`#content`）で何も起きない | ✅ 実測 |
| **BFCache 復帰（`pageshow` persisted）** | ⚠️ ユニットテストのみ |
| **同一ドキュメント内の「戻る」**（reload が commit する前に戻る） | ⚠️ ユニットテストのみ |
| **backstop（12 秒）の再読み込み提示** | ⚠️ ユニットテストのみ |

下 3 つは**タイミングを外から作るのが難しい**ため実機で踏めていません。
同一ドキュメント内の戻るは「reload が投げられてから commit するまで」の窓で
戻る必要があり、その窓はローカルでは数十 ms です。

**これは前回の実装（PR #75、クローズ済み）が 4 ラウンドかけて落とし続けた場所**
なので、ユニットテストだけで閉じたとは言いません。

- **どう確かめるか**: (a) `verify:ui` 相当のスクリプトで、CDP のネットワーク
  スロットリングを使って reload を遅くし、その窓で `history.back()` を叫ぶ、
  (b) backstop は `backstopMs` を注入できるので、テスト用ビルドで 300ms にして
  実際に retry ボタンを押す
- **完了の定義**: 3 経路とも実ブラウザで踏み、veil が残らないこと・
  retry が実際に再読み込みすることを確認した記録がある
