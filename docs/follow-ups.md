...
### F-201 `higher-brain-function` の phone レイアウトに 13 件、公開シーンは 1 件 — P2（2026-09-22）

F-162 を直して（`verify:ui -- --scene <slug> --preview`）、この非公開シーンを
初めて 390px で**測りました**。結果は **13 件**で、比較用に同じ条件で測った
公開シーン `brain-anatomy` は **1 件**です。

**どれも今回の改修が原因ではありません**——全部が共有 chrome の問題で、
このシーンの**状態（alpha）とコントロールの数**が引き金になっています。

| 件数 | 内容 | なぜこのシーンだけ出るか |
| --- | --- | --- |
| 9 | 操作バーのボタンが 37〜40×38px（44px 未満） | このシーンは 9 個並ぶ（brain-anatomy はより少なく、縮まない） |
| 3 | stage stepper が spatial inspection パネルに覆われる | brain-anatomy は stage を持たない |
| 3 | 属性が英語のまま（alpha バッジ、レビュー状態、観察を閉じる ×） | brain-anatomy は `production` なので alpha バッジが出ない |
| 1 | コントロールを隠すと、戻す唯一のコントロールも隠れる | 同上（stage 系） |

- **読み取り欄そのものは問題として出ていません。** 今回 19 → 26 行に増えましたが、
  実測で **193px 幅・はみ出し 0px・最も広い行 171px**（`shots:phone` に
  `8-readout` を追加して測りました）。ただし**phone ではパネル自体が
  画面外**にあり、絵の中に写っていません——これも測れていない面です
- **絵で 1 件見つかりました（既存）**: 経路のラベル
  「音韻の出力計画（Broca 野：下前頭回弁蓋部・三角部）」が **390px で左端から
  はみ出して切れています**。ラベル層は `position: fixed` のオーバーレイなので
  `scrollWidth` に出ず、`verify:ui` のはみ出し検査に**かかりません**。
  ノードのラベル自体は今回変えていないので既存の問題です。
  **これも「名前で見ていない検査」の穴**（L-54 の形）
- **やるなら**: 操作バーのボタンは共有コンポーネントなので、直すと全シーンに
  及びます。「1 件直したら 1 本出す」に従って別 PR。alpha バッジの英語属性も同じ
- **やってはいけないこと**: このシーンだけに例外 CSS を書くこと

### F-202 読み取り欄が 27 行あり、グループ化も開閉もない — P3（2026-09-22）

課題が 22 行＋補助 5 行です。行そのものは短い（多くは 7 文字）ので
はみ出してはいませんが、指示書 §13.3 が求める**グループ化・開閉**は
まだありません。

- **いま抑えていること**: 長い散文の行（「この値が決めていないこと」）は
  **1 件だけ**表示し、複数あるときは「（N 件のうち 1 件）」と前置きします。
  全部連結すると 215 文字になり、190px の欄では読まれずスクロールされます
- **やるなら**: 「話し言葉」「文字」「出力」などでまとめて開閉する。
  ただし**対象外の 3 行を畳んで隠さないこと**——「無い」ことが所見です

### F-198 概念的な連絡と実トラクトが、3D 上で見分けられない — P2（2026-09-22）

モデル側は各連絡が `mapping`（`atlas-structure` / `coarse-educational` /
`conceptual`）を宣言するようになりました。**画面はまだ区別していません。**
弓状束（実メッシュ）と、書記素の保持へ向かう概念的な連絡が、同じ 1 本の
チューブとして描かれます。

- **なぜ問題か**: 読者に「患者の tractography を見ている」という印象を与える
  一方、実際には一部が機能上の矢印です。指示書の §10.4 と §13.4 が禁じている形
- **やるなら**: `mapping` で線種を変える（実トラクトは実線、概念は破線など）＋
  凡例に 1 行。色だけに依存させないこと
- **決めていないこと**: 概念モードのときに 3D をどう変えるか。いまは読み取り欄の
  1 行目だけがモードを言い、**絵は同じ**です

### F-199 古典症候の参照パネルが、まだ画面に出ていない — P2（2026-09-22）

`src/data/aphasiaReference.js` に 8 症候＋3 つの「失語ではない」像を
参照情報として置きましたが、**UI からは読んでいません**（モデル側から
import できないことはテストが固定しています）。

- **いまの状態**: データはあり、テストもあり、画面に出ていない。読者は
  「経路の値」だけを見て、比較する相手を持ちません
- **やるなら**: 展開できるパネル 1 枚。一致率・確信度・パーセンテージを出さない、
  複数該当と該当なしを許す、未評価項目を明示する——指示書 §9.2 の制約に従う
- **注意**: パネルを作るとき、`tests/aphasia-reference.test.js` の
  `reference: no scene or app file imports it as a source of answers` が赤くなります。
  **そのテストが「表示のために import した」と「計算に使った」を区別する形に
  書き換わることが、パネルを入れる条件です**

### F-200 書字 2 経路の局在が確度の低い根拠に乗っている — P2（2026-09-22）

語まるごとの綴りを優位側角回、音韻—文字変換を優位側縁上回に置いています。
根拠は 1984 年の CT 症例群（各群 4 例）で、メッシュは脳回まるごとです。

- **確かめていないこと**: 本文未取得（この環境から医学出版社に到達できません）。
  後続研究がこの局在をどう扱っているかも未確認
- **やるなら**: 本文が読める環境で照合し、`sourceVerification` を更新するか、
  局在を概念ノードへ落とす。**機能としての 2 経路の解離は別の主張で、
  そちらは残ります**
- **いま書いてあること**: evidence registry で `uncertain`、model card の
  §10 と dossier §3 に明示
