...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-187〜F-196** | 失語・言語経路モデルの再設計（使うのは F-187〜F-190。**採番し直し 2 回目**: F-167 → F-177 と動かしたが、どちらも main が先に取っていました） | 2026-09-22 |
  | **L-81〜L-90** | 同上（使うのは L-81〜L-83。**L のブロック予約は main の指示どおり**——下の 15 回目参照） | 2026-09-22 |
...
### F-191 `higher-brain-function` の phone レイアウトに 13 件、公開シーンは 1 件 — P2（2026-09-22）
...
### F-192 読み取り欄が 27 行あり、グループ化も開閉もない — P3（2026-09-22）
...
### F-188 概念的な連絡と実トラクトが、3D 上で見分けられない — P2（2026-09-22）
...
### F-189 古典症候の参照パネルが、まだ画面に出ていない — P2（2026-09-22）
...
### F-190 書字 2 経路の局在が確度の低い根拠に乗っている — P2（2026-09-22）
...
### F-187 15 秒シーケンスの実レンダリング — 見た。残りは 2 点 — P3（2026-09-21）
...
（`docs/verification-lessons.md` L-81）、(2) 「どこで止まったか」が
