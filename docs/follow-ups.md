...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-159〜F-168** | 脳アトラスの配色を大分類ごとに揃える（この PR。使うのは F-159 のみ） | 2026-09-19 |
...
### F-159 大分類ごとに揃えた配色を、色覚特性のある読者で確かめていない — P2（2026-09-19）

脳アトラスの colour map を組み替え、**大分類（葉・脳室系・小脳・脳幹）ごとに
1 本の狭い色相帯**を与えました（`DETAIL_COLOR_FAMILY`、
[`organ-3d-playbook.md`](organ-3d-playbook.md) §2 P）。これで
**グループの識別は色相に、メンバーの識別は明度・彩度に**寄りました。

- **なぜ未確認なのか**: この移動は色覚特性のある読者にとって**良くも悪くもなり得ます**。
  メンバー間が明度差になったのは有利ですが、**グループ間の区別が色相だけに
  なった**ので、たとえば 2 型（deuteranopia）では前頭葉（橙）と頭頂葉（黄緑）が
  近づきます。**どちらに転んだかは測っていません**
- **何を測るか**: 主要な 3 型（protan / deutan / tritan）のシミュレーション下で、
  (1) 皮質葉どうしの最小 ΔE と (2) 各 family 内の最近傍 ΔE。いまの guard
  （`tests/brain-anatomy.test.js` の全ペア ΔE ≥ 3.8 と色相帯）は
  **正常色覚の Lab 空間でしか測っていません**
- **どこに足すか**: 同じテストに変換を 1 つ足せば済みます——`hexToLab` の前に
  シミュレーション行列を挟むだけで、新しい道具は要りません
  （[CLAUDE.md](../CLAUDE.md)「同じ車輪を二度作らない」）
- **決めていないこと**: 落ちたときに配色を動かすのか、
  **色以外の手がかり**（ラベル・輪郭・選択時の強調）で足りるとするのか。
  現状の scene は選択・hover で名前を出すので、色は入口であって唯一の手段ではありません
- 完了の定義: 3 型それぞれの下限が測られ、閾値がテストに書かれていること
