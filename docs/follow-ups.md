...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は **F-137** です（F-111〜F-129 は main 側で採番済み。F-130〜F-135 も
  main 側、F-136 はこの PR。
...
- **未確認 1: 他エンジン。** `final-browser-validation.yml` は Firefox / WebKit も
  回すので、この PR の `verify:ui` がそこで緑になることを見てください。
  ローカルで回したのは Chromium の scene surface × 3 viewport
  （`desktop-1280` / `phone-320` / `phone-landscape`）で、**残り 4 viewport は未実行**。
...
- **未決定: 2.2 秒という値。** 動画プレイヤーの慣習（2〜3 秒）から取っただけで、
  ...
  `UI_QUIET_MS` 1 か所です。
- 完了の定義: CI の 3 エンジンで `verify:ui` が緑、実機のタッチ端末で
  「下がる → 触れて戻る → 押して UI が戻る」を 1 回通し、待ち時間の値を決める
  （変えるなら 1 か所）。
