...
**これは下限です。** 格子はキャンバス中央だけで、刻み（48×38 px）より細い構造は
2 点のあいだに落ちます。「少なくとも N」と書き、「全部で N」とは書きません。
...
- **膀胱は指せません。** 枠の外です。3 回目の表はこれを 5 のうちに数えていました
- **C が 3 案で最良**（9）。錐体を名前で指せるのは C だけです。
  B は中を開きますが、**内部の 4 構造が 221 点のうち 1 点ずつしか当たりません**
  ——1 刻み（48 px）より小さいということです
- **どの状態でも、模型はキャンバスの 12〜17% しか占めていません。**
  構図の選択より先に、**腎が画面で小さすぎる**という問題があります
