...
**この項目の初出は「#112 のあと」と書いていました。測り直したら違います。**
同じ計器（現行の `verify:anatomy`）で、#112 の前後のビルドと、公開中の臓器を
並べて測った結果:

| シーン | #112 前 | #112 後 | 幅 |
| --- | --- | --- | --- |
| knee-anatomy | 0.42..0.58 | **0.32..0.42** | 0.16 → **0.10** |
| shoulder-anatomy | 0.42..0.58 | **0.28..0.38** | 0.16 → **0.10** |
| liver-anatomy（公開） | — | 0.22..0.48 | **0.26** |
| lung-anatomy（公開） | — | 0.22..0.50 | **0.28** |

読み方は 2 つあり、**片方だけが本当の指摘です**。

- **左へ寄ったのと、全体が細くなったのは #112 の意図どおり。** パネルが覆う右を
  除いた帯に合わせるようになったので、canvas に対する割合は帯の取り分
  （約 0.72）だけ下がります。0.16 × 0.72 ≈ 0.115 で、実測 0.10 とほぼ合います。
  **#112 の退行ではありません**
- **本当の指摘は、同じビルドの公開臓器と比べたときです。** 肝 0.26・肺 0.28 に対し、
  膝と肩は **0.10**。**2.6 倍細い。** これは #112 の前からある差で、
  #112 はそれを目立たせただけです

原因は subject の中身です。膝は「2 本の骨幹とその間の関節」、肩は
「鎖骨・肩甲骨・上腕骨」——**長骨の軸を含むので縦に長く、高さで枠が決まり、
幅が余ります**。`KneeAnatomyScene` の `minHorizontalAspect: 0.45` に
「widest whole view fills the frame's width at an aspect of 0.42」と
書いてあるとおりで、意図された数字が、意図されなかった帰結を生んでいます。
...
- やること: **フィットさせる subject から骨幹の遠位を外す**か、関節を中心に寄せる。
  骨幹を消すのではありません——読者は「どこの関節か」を骨の向きで読むので、
  幹は見えている必要があります。**枠を決める箱と、描くものを分ける**話です
  （`getSubjectBounds` は今その区別を持っていません）
- 比較の基準は公開臓器の 0.26〜0.28。**膝と肩がそこに届けば F-134 は終わりです**
- 完了の定義: 膝と肩が、他の臓器と同程度に帯を占めること。
  `verify:anatomy` の `spans` 行が指標になります（この行自体が #112 の産物）。
  **そのうえで肩の名前つき tour を 3 点以上に戻すところまで**が F-134 です

**一部解決（2026-09-16）。** 直し方は `getSubjectBounds` に新しい区別を足すことでは
ありませんでした——**`contextTags` として既にあります**（腎が膀胱まで届くのを外すのに
使っているのと同じ仕組み）。骨幹に `shaft` タグを付け、両シーンで
`static contextTags = ['shaft']` を宣言しただけです。

| | 前 | 後 | 公開臓器 |
| --- | --- | --- | --- |
| shoulder | 0.10 | **0.18** | 0.26〜0.28 |
| knee | 0.10 | **0.14** | 0.26〜0.28 |

**骨幹は今も描かれています。** 枠を決める箱から外しただけで、
大腿骨は上から、上腕骨は下へ、これまでどおり画面外へ伸びます——
読者は骨の走行で「どこの関節か」を読むからです。

効果は幅だけではありません。**名前つき tour が持てる構造の数が増えました**:

- 膝: 3 点（すべて同じ列）→ **4 点 4 構造**（列も行も分かれる）
- 肩: 実質 **1 点**（4 候補のうち 3 つが「関節唇」に落ち、2 点は run ごとに
  答えが変わった）→ **3 点 3 構造**、2 run 連続で一致

**まだ公開臓器の 0.26 には届きません。** 残りの制限は subject そのものの縦横比です——
骨幹と腓骨を外しても膝は 1.88 × 3.34（縦横比 0.56）で、
上端は大腿四頭筋腱（+2.03）、下端は内側側副靱帯（−1.31）。
どちらも関節の一部なので、これ以上は「箱から外す」では動きません。
次に効く手は camera pose か coverage で、**どちらも構図の判断**です。

- 完了の定義（変更なし）: 膝と肩が公開臓器と同程度（0.26〜0.28）帯を占めること
