...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は F-109 です
  （F-101〜F-106 は Landing hero の PR #72、F-107 はシェル動線の PR #73、
  F-108 はこの PR #75 が確保済み。いずれも未マージなので、先に入った側の
  番号がファイルに現れます——重複しないことだけが約束です）
...
### F-108 遷移中の表示：直したこと、直していないこと — P2（2026-09-14）

モデルを描いている面からハッシュでルートを移ると、リロードが commit される
までブラウザは前のドキュメントを描き続けます。つまり**新しい URL の下に前の
モデルが残る**。「共有された COPD のリンクを押したら脳のモデルになる」と
報告されたのがこれです。`leaveForReload()`（`src/app/sceneShellBridge.js`）が、
イベントと同じタスクで画面を覆ってからリロードを要求します。

**監査で見つけて同じ PR 内で直したもの**（最初の実装は 4 点とも落ちていました）。

- **veil が二度と消えず、ラッチも解放されませんでした。** リロードが commit
  されない場合——リロード中に Back、あるいは bfcache からの復帰——古い
  ドキュメントが不透明でクリックも通さない veil に永久に覆われます。
  **直そうとした問題より悪い状態**でした。`pageshow` の `persisted` で解除します。
- **`z-index: 20` では足りませんでした。** 上に描かれるものが 6 つあります
  （reel chrome 30 / feedback trigger 35 / consent banner・access modal・
  landing nav・スマホの部位シート 40 / feedback overlay 60 / skip link 100）。
  効いたのは部位シートで、**モデル自身の部位リストが新しい URL の下に残り、
  押せる**状態でした。120 に上げ、`tests/leave-for-reload.test.js` が
  `src/styles/` を列挙して、これより上に何も現れないことを固定します。
- **シーンの面しか直していませんでした。** トップページの hero と `#/organs` の
  hero は**どちらも脳**で、素の `reload()` のままでした。**今回の報告の
  最有力経路はこちらです**（`main.js` の landing / explorer ハンドラ）。
- **後続の遷移を飲んでいました。** ラッチが 2 回目の `hashchange` を no-op に
  していたため、遅い遷移中に Back を押すと握り潰され、最初の行き先が commit
  される＝アドレスバーと中身が食い違います。2 回目以降も `reload()` を
  呼び直します（`reload()` は「いまの」アドレスバーを読むので、最後の行き先が勝つ）。

**2 周目の監査で見つけて直したもの。** 上の 4 件を直したコミットに、さらに
3 件ありました。同じ種類の誤り——「自分が書いた仕組みが動くこと」だけを確かめ、
それが失敗する条件を確かめていない——の 3 周目です。

- **veil を下ろせるのが persisted `pageshow` だけでした。** このアプリはハッシュ
  ルータなので、**Back は同一ドキュメント遷移で `pageshow` を発火しません**。
  Stop も同じです。つまり「Back で取り残される」という、直したと書いたまさに
  その経路が残っていました。**実装中に自分でこの穴に気づいていながら、コミット
  メッセージには逆のことを書いています。** いまは経路を数え上げるのをやめ、
  `DEPARTURE_BACKSTOP_MS`（30 秒）の backstop で、イベントが 1 つも来なくても
  veil が下りるようにしました。長いのは意図的で、**誤って発火しても「この関数が
  無かった頃の挙動」に戻るだけ**——バグより悪くならないことが、状態機械ではなく
  ただのタイマーで済ませてよい理由です。
- **「最後の行き先が勝つ」が、元のルートへ戻るときだけ効きませんでした。**
  `currentHash` は mount 時の 1 回きりで更新されないため、`sameRoute` が true に
  なり `leaveForReload()` に到達しません。呼び出し側 3 箇所が `isLeaving()` を
  先に見るようにしました。
- **z-index ガードが 29 枚中 9 枚しか見ていませんでした**（z-index を宣言する
  14 枚のうち 9 枚）。実害はありませんでした（未検査の最大値は 12）が、
  **コミットメッセージとこの項目に「全スタイルシートを走査」と書いていました。**
  事実と違う保証です。ディレクトリ列挙に変え、枚数と検査した宣言数も assert します。

ガードは 4 つとも変異テストで確認しました（z-index を 50 に下げる／`isLeaving()`
を外す／backstop を外す／再要求を外す——どれも赤になります）。

**3 周目の監査で見つけたもの。** 5 件あり、うち 1 件で**ようやく元の報告が
再現しました**。

- **シーン起動中は `hashchange` ハンドラがどこにも存在しませんでした。** シーンの
  ハンドラは `createApp()` の中、つまり**数 MB のアトラスを読み終えた後**にしか
  付きません。その間にリンクを押すと URL だけが変わり、**veil も出ずリロードも
  요求されず**、そのあとモデルが新しい URL の下に描画されます。
  production ビルドで A/B を取りました。

  | | hash | route | canvas | h1 |
  |---|---|---|---|---|
  | ガードなし | `#/copd` | `scene` | **true** | Interactive brain anatomy |
  | ガードあり | `#/copd` | `locked` | false | COPD |

  **これが報告された症状そのものです。** 1 周目も 2 周目も、読み込み済みの
  ページしか見ていなかったので、両方とも別の窓を直していました。
  `main.js` のシーン分岐の先頭でガードを張ります。
- **backstop が再要求で張り直されていませんでした。** 1 回目の 30 秒が新しい
  リロードの最中に切れて、古いモデルが最新の URL の下に戻ります——安全網が
  バグを復元する形です。テストが無かったので（外しても緑でした）追加しました。
- **`role="status"` が機能していませんでした。** live region は*変化*を読み上げ
  るので、本文を入れてから append すると何も announce されません。空で
  append してから本文を入れます。
- **テスト 5 件が本物の 30 秒タイマーを張っていました。** このファイル 1 本で
  `npm test` に **30 秒**追加していました（実測 30120ms → 107ms）。
  pass/fail しか見ていなかったので気づいていませんでした。
- **完全性の assert が数を取り違えていました。** 宣言数（37）を枚数（14）と
  比べていて、何も固定していませんでした。

**直していないこと（意図的）。**

- **Trust・法務文書・未公開ページ・失敗フォールバックは素の `reload()` のまま**
  です。残るのは静的なテキストで、行き先と取り違えようがないからです。
  一貫性のために揃える判断もありえますが、いまは変えていません。
- **元の報告は再現できていません。** production ビルドを素の静的サーバで
  配信し、クリーン／保存済みアンロック／`?preview=1` の各条件で `#/copd` を
  開きましたが、すべて正しく COPD の準備中画面になりました。この PR は
  「開いていたタブで踏んだ」経路を塞ぐもので、「ハッシュが届いていなかった」
  可能性は残ります。**判別にはその瞬間のアドレスバーが要ります。**
- **WebKit / Firefox は未検証です。** F-100（Actions 無料枠）で CI が使えません。
  Chromium・production ビルドの 375px で、4 面すべて veil が最前面に出ることは
  実測済みです。
- 完了の定義: アドレスバーの確認で再現経路を確定し、CI が戻ったら
  `final-browser-validation` を通して本項を Resolved へ。
