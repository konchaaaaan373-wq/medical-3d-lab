...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-187〜F-196 / L-81〜L-90** | 心拍出量シーン（`cardiac-output`）の追加。使うのは F-180〜F-184・F-187・F-188 と L-81〜L-93（L-91〜L-93 は予約幅 10 の外側で、main の最大は L-72 なので衝突しません）。**採番し直し 2 回目**: L-64・L-65 → L-71・L-72 → L-81 以降、F-177〜F-186 → 空きを残して F-187 以降。どちらも main が先に取っていました | 2026-09-22 |
  | **F-197〜F-206 / L-94〜L-103** | ナビゲーションと App Shell の UX 監査（reading surface 間の document 再読込み廃止・共通ヘッダ・Trust の IA・boot veil）。使うのは F-197〜F-202 と L-94〜L-96 | 2026-09-22 |
...
### F-197 — シーン間の移動は、いまも document の載せ替えのまま
...
### F-198 — `#/trust` の中でモデルを変える deep link は何も起こさない
...
### F-199 — `visually-hidden` 相当の規則が 6 つの stylesheet に別々にある
...
### F-200 — 公開ビルドの `#/organs` は、いまもホームとほぼ同じページ
...
### F-204 — 「未解決の限界」の本文が、日本語の見出しの下で英語のまま
