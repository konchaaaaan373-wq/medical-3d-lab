...
### F-212 — 心拍出量の「実験装置」レイアウト: 実機未確認と、決めていない 4 点 — P2

`cardiac-output` を `meta.layout = 'experiment'` に移しました（状態 → 介入 → 心臓 →
数値の順に配置、4 本のスライダーは「詳細パラメータ」の奥、カメラ・観察・リール・画像は
「その他」の奥、読み出しは「操作前 → いま ＋Δ」）。確認したのは **headless Chromium の
1440×900・1280×720・1024×768・390×844** と `verify:disease` までです。

**未確認**

1. **実機（タッチ・実 GPU）と production URL**。software GL では orbit の慣性が
   秒単位で残るため、「介入の前後で同じ視点」は**ドラッグしていない状態**でしか
   測れていません（`verify:disease` の experiment layout 節）
2. **1024〜1100px 幅**では「詳細パラメータ」を開くと、パネルが心臓の左端に重なります
   （1440 では重ならない）。幅に応じて下に置くか、狭い幅でだけ重なりを許すかは未決定

**決めていないこと**

3. **スライダーを手で動かしたとき、② 介入の行は「なし」が選ばれたまま**です。
   先頭行は「手動で変更」と言いますが、② の行に「手動」の状態はありません。
   足すなら選択肢ではなく状態表示です（押せる「手動」は意味を持たない）
4. **状態を切り替えたときは Δ が出ません。** 状態は「実験の出発点」で、選ぶと
   比較の基準（baseline）がその状態に取り直されるためです（`ExperimentSession`）。
   「基準心 → 収縮力低下で CO がどれだけ下がるか」を見せたいなら、それは
   baseline の意味を変える**モデル側の判断**で、この UI 変更には混ぜていません
5. `circulation` も同じ形（状態を 1 つ選ぶ）ですが、既定のレイアウトのままです。
   移すかどうかはそのシーンの読み出しの設計次第
6. **「学ぶ」を開くと**、console が lesson の分だけ上に伸び、Data view の
   左の圧波形パネルの下端（凡例の行）に重なります（1440×900）。操作には支障なし
