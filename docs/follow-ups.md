...
- **F-137 WebKit だけ、tablet-768 でコントロール列が横にはみ出す** — 解決（2026-09-17）。
  ...
  **`nowrap` である限り、箱が足りない分はそのまま画面外です。**
  ...
  確認（Chromium で WebKit の条件を再現）: コンソールを 251px に強制すると、
  行は箱 231 / 中身 231 の **2 行**になり、6 つのボタンは全部箱の内側。
  そのままなら 265 / 245 の 1 行で、以前と同じです。
  Chromium の 7 viewport × scene は全部緑。WebKit の結果は下の「まだ確かめていない」を参照。
  **残っている懸念**: 431〜560px はスクロールのままなので、
  コントロールがもっと多いシーンが来ればそこでは同じように黙ります。
  帯が狭く、いま該当するシーンは無いので cue は足していません。
