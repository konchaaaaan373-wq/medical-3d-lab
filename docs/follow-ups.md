...
1. **実機での受け入れ確認** — 2026-09-25 に iPhone（Safari、production）の画面収録を
   1 本受け取りました。状態・介入・Δ・回転は期待どおりでしたが、**心臓がパネルの裏に
   2 倍の大きさで写っていました**（L-114。framing の下限割れ。直しました）。
   まだ見ていないもの: 修正後の実機、Android、タブレット、横向き
2. **375×553（iPhone SE + Safari のツールバー）では、心臓が読み出しと console に
   上下 30 px ずつ潜ります。** 帯が 41 px しかなく、fit しても入りきりません。
   直すなら電話での読み出しの畳み方（例: 先頭行だけ残す）の設計判断で、
   `verify:disease` は報告のみにしています
