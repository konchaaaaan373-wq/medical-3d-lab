...
- **2026-09-12 の変更点**: 受信箱が要る部分**以外**は自動化しました。
  ...
  （`?account=recovery` だけを頼りに）同じフォームが出ることを実測します。
...
| ~~`elbow-anatomy`~~ | ~~「recolouring left 0 rows marked selected」1 件のみ~~ | **解決（2026-09-14 / B3）。製品ではなく検査の側でした**——下の F-107 を参照。いまは 4 点が別々の構造に当たります（上腕骨・上腕二頭筋腱・上腕三頭筋腱・関節包） |
...
- **残りは `hand-anatomy` だけです。** 3 件とも `verify:anatomy` は通りますが、
  **手が通るのは「外した点を測り直す」からです**（F-107）。記録済み 4 点のうち
...
### F-107 ブラウザ検査が、自分の狙いのずれを製品の欠陥として報告していた — 解決（2026-09-14 / B3）
...
### F-108 安全域フィットが遠近を無視していた——奥行きのある被写体が枠から出る — 解決（2026-09-14 / B3）
...
### F-109 眼と耳の解剖シーンは、まだ公開の水準に無い — 一部解決（2026-09-15） / P2（未公開）
...
### F-110 開いた直後の構図と、「表示をリセット」後の構図が違った — 解決（2026-09-15）
...
- F-107 — 2026-09-14 — ブラウザ検査が、開始状態で測ったクリック座標をカメラが
...
- F-110 — 2026-09-15 — 読み込み直後の構図が、シェルがまだ印を付け終えていない
...
- F-104（`elbow-anatomy` と `lymphatic-drainage`） — 2026-09-14 — 肘は検査側の
  欠陥（F-107）で、直したら 4 点が別々の構造に当たるようになりました。リンパ系は
  記録済みの点がそのまま通ります。**`hand-anatomy` は開いたままです**——通るのは
  再照準のおかげで、構図そのものは直っていません。

- F-108 — 2026-09-14 — `fitPoseToSafeArea` が透視カメラに対して正射影の計算を
