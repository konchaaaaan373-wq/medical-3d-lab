...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-159〜F-168** | 病態シーンの動画書き出しと同意画面（使うのは F-159〜F-164） | 2026-09-21 |
...
### F-161 動画書き出しを確かめたブラウザは 1 つだけ — P2（動画書き出し）
...
### F-162 `verify:ui` は reel mode を開かないので、その面は測られていない — P2（動画書き出し）
...
### F-159 動画にできるのは「シーケンスを持つ 5 シーン」だけ — P3（動画書き出し）

動画書き出しは reel（15 秒のシーケンス）を持つシーンにだけ現れます。いまそれは
COPD・喘息・心不全・門脈圧亢進・肝腎症候群の 5 件で、**他の病態シーンは
アニメーションしていても書き出せません**。`videoExportProblems()` は
...
### F-160 書き出しの解像度は canvas のバックストアで、1080×1920 ではない — P2（動画書き出し）
...
### F-163 16:9 プリセットのカードがモデルに重なる（画面・ファイルの両方） — **対応済み**
...
### F-164 書き出しのコードは、使えない production ビルドにも載っている — P3（動画書き出し）
