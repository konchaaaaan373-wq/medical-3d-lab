...
### F-213 — 心拍出量: 階層表示と、閉じた 2 枚の操作カード — 実機・初見が未了 — P2

2026-09-26: 左上を「病態モデル › 心拍出量」に（「病態モデル」は `#/pathology` へのリンク）、
右の操作欄を「条件を変える」「見え方」の 2 枚のカードにしました。どちらも最初は閉じていて、
開けるのは 1 枚ずつ。「見え方」には拍動の表示速度（通常・1/4・止める。表示だけ）・比較・
グラフ・視点・観察を置き、レッスン・リール・画像は「その他」に残しています。
詳細はモデルカード「Screen, 2026-09-26」。

**区別**: 実装済み・単体テスト済み・headless Chromium でのブラウザ確認済み（エミュレーション）。
**実機確認・初見の利用者の確認は未了**です。

**残っていること**

1. **初見の利用者が、閉じたカードの中に操作があると気づくか。** 見出しと 2 行目
   （「収縮力・心拍数・充満量・抵抗」）だけが手がかり。F-212 の課題 1〜5 を、カードを
   閉じた状態から始めてやってもらう（未実施）
2. **電話で「条件を変える」を開いている間は、「見え方」の見出しを出していない。**
   見出しの行をパッドの切り替えに使っているため（行を 1 本増やすと 390×664 で左室が
   77 → 49 px になった）。見え方へは、条件のカードを閉じてから 1 タップ。これで困るかは未確認
3. **拍動の表示速度**を 1/4 にすると、パッドの「70 /min」より遅く拍動して見える。
   ボタンの下の注記だけが理由を言っている。誤読されないかは未確認
4. `#/pathology` は最小の一覧（公開ゲートが開けた病態モデルのカードだけ）。サイト全体の
   ナビ（ホームのヘッダ）からはまだ辿れず、入口は機序シーンのパンくずだけ
5. 実機（iPhone Safari・Android Chrome）で、カードの開閉とカメラの合わせ直しが
   暴れないか。Safari のツールバー伸縮の下で `:has()` による見出しの畳み方が崩れないか

**実測**（preview ビルド・headless Chromium・`verify:disease`、左室の高さ）:

| 画面 | カードを閉じた最初の画面 | 「条件を変える」を開いて操作した後 |
| --- | --- | --- |
| 1440×900 | 339 px | 334 px |
| 390×844 | 159 px | 143 px |
| 390×664 | 174 px | 83 px |
| 375×667 | 168 px | 85 px |
| 375×553 | 108 px | 37 px（F-212 の未解決のまま） |
| 320×568 | 97 px | 開くと console が心臓に 212 px 重なる（報告のみ） |

閉じた状態では短い電話でも左室が読める大きさになった。**開いた状態は前回（常時表示）と
ほぼ同じで、375×553 と 320×568 は未解決**。320 px ではパッド切替の値を 2 行に折り返す
（`verify:disease` が全範囲の両端で切替の値が欠けないことを検査する）。
