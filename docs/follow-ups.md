...
### F-125 教訓の 9 件は、まだ人しか捕まえられない — P2（2026-09-16）

`docs/verification-lessons.md` が 17 件を持ち、`npm run lessons`（`npm test`
からも走る）が台帳の側を守ります——3 つの欄があること、**名指ししたガードが
実在すること**、まだ人しか捕まえられない件数。

**17 件中 9 件が「人だけ」です。** 内訳は L-01 / L-04 / L-05 / L-06 / L-09 /
L-13 / L-14 / L-15 / L-16。このうち機械化できそうなのは:

- **L-01・L-04・L-06**（不在の主張が空集合に通る / 正規表現が別の規則に当たる /
  ソース検索がコメントに当たる）——共通のヘルパーに寄せれば一度に減ります。
  `tests/locked-surface.test.js` の `sizeOf()` が既に正しい形を持っており、
  `scripts/type-floor.mjs` と `tests/surface-polish.test.js` が同じ CSS 解析を
  手書きで持っています。**3 本目が生えているので、寄せどきです**
  （CLAUDE.md「同じ車輪を二度作らない」）
...
- どう確かめるか: `npm run lessons` の「9 of 17」が下がること。
  **項目を消して下げたら逆走です**（テストがその数がゼロでないことを主張しています）
- 完了の定義: 上の 4 件に機械的なガードが付き、台帳の該当欄が
