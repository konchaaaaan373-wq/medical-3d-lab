...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は **F-143** です（F-111〜F-129 は main 側で採番済み。F-130〜F-135 も
  ...
  **番号はマージ直前に取り直してください**（L-16）——台帳の冒頭に
  ...
  ⚠ **8 回目（2026-09-17）**: 脳の医学レビュー対応 branch が F-120〜F-124 を
  使っていましたが、main が同じ番号を別件で先に使っていたため、branch 側を
  **F-138〜F-142** に動かしました（旧 F-120→F-138、F-121→F-139、F-122→F-140、
  F-123→F-141、F-124→F-142）。そのままの番号で残っている `docs/clinical-reviews/`
  の AI レビュー記録本文は、レビュアーの原文なので書き換えず、各記録の冒頭に
  対応表を置いています。
...
### F-138 脳解剖：人間の臨床医・解剖学者による監修署名がまだ要る — P1（2026-09-16）
...
### F-140 上流由来の片側だけのメッシュ（延髄） — 原因不明、鏡像複製はしない — P2（2026-09-16）
...
後横側副溝の別の不具合（右メッシュのノード名の先頭スペース）は F-142 を参照。

### F-142 「Posterior transverse collateral sulcus」ノード名の先頭スペースと検索 0 件 — P3（2026-09-16）
...
### F-141 Najdenovska／CIT168 由来の統合区画に、元ラベル id の対応記録がない — P2（2026-09-16、一部解消）
...
——同じ衝突の 8 回目です。**件数は `npm run lessons` の出力から書くこと。**
動かしたのは #122 側です。#123 側の L-31〜L-33 は `src/app/App.js`・
`scripts/capture-anatomy-views.mjs`・`tests/hide-interface.test.js` から
指されており、docs だけで閉じる方を動かす方が安全でした。

**ガードはあり、働きました——ただし遅れて。** `npm run lessons` は重複番号を
名指しで落とします。しかし**両方がマージされるまで赤になりません**: #122 も
#123 も単独では緑で、main が赤くなったのは 2 本目がマージされた
run 640（2026-09-17 04:37、head `4b6cac8`）です。**そこから、この修正が
入るまでの間、main から切った全 PR の `npm test` が、その PR とは
無関係な理由で赤くなります。**
番号の衝突を「マージ前」に落とすには、branch protection の
**Require branches to be up to date before merging** が要ります——
CI の分数と引き換えなので、private 化のとき（F-138）に一緒に決めてください。）
...
- **F-139 16 枚のレンダリング画像と単独選択画面をレビューアへ送る** — 解決（2026-09-17）。
