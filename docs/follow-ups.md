...
- 直し方の見当（**未確認**）: 断面平面を腎の tag にだけ適用する（尿管・膀胱を
  除外する）か、切断面に cap を張る。**前者のほうが小さく、この scene が既に
  `contextTags` で tract を別扱いしている構造に乗ります。** 測ってから直すこと
- 完了の定義: `coronal-section` ビューで尿管が上端から下端まで同じ太さの管として
  描かれ、実レンダリングで確認できる
