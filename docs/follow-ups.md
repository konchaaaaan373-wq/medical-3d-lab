...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-159〜F-168** | 脳アトラスの配色（大分類ごとの統一と色覚配慮。F-159 は解決済み、F-160 を使用） | 2026-09-19 |
...
### F-160 色覚の確認はシミュレーションどまりで、当事者が使っていない — P2（2026-09-20）

F-159 で配色を 4 視（normal / protan / deutan / tritan）の床に載せ、
...
- **床はあえて低くしてあります（2026-09-21 改訂）。** 一度 4 視すべてで ΔE 12 以上まで
  ...
  差し戻しました（[L-52](verification-lessons.md)）。いまの床は**表面 8 family で
...
- **F-159 大分類ごとに揃えた配色を、色覚特性のある読者で確かめていない** — 解決（2026-09-20）。
