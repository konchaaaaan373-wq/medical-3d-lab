...
- **番号は Resolved も含めたファイル全体の最大値の次から取ります。**
  ...
  次に追加する番号は F-124 です（F-108〜F-110 は 2026-09-14〜15、F-111〜F-119 は
  09-15、F-120〜F-123 は 09-16 に採番済み）。⚠ **F-107 / F-108 も 2 回ずつ出ます。** 未マージの PR #73 と
...
### F-121 16 枚のレンダリング画像と、単独選択した画面をレビューアへ送る必要がある — P1（2026-09-16）

2026-09-16 のAI照合は、依頼で用意されたはずの 8視点×2配色 16 枚のモデル画像を
**1 枚も受け取っていません**（本文 §2.1、「モデル画像の閲覧数は0枚」）。
GLB の 271 メッシュそのものの照合も未実施です。レビュー §3.2 条件 3 は、
16 ビューに加えて**問題となった構造を単独選択した左右別の画面**を人間の
臨床医・解剖学者に見せることを次の条件としています。

- どう確かめるか: 指摘のあった構造（大脳脚底 #18、中心下回 #59、視床の
  Najdenovska 7区画、扁桃体・視床下部の統合区画、片側のみの延髄・後横側副溝など）
  を単独選択した画面と、8視点×2配色の 16 枚を、実際にレビューアへ送付する。
- 完了の定義: 画像がレビューアに渡り、その受領と所見が記録に残る
  （次の版のレビュー記録、または本項の更新として）。

### F-122 上流由来の片側だけのメッシュ（延髄・後横側副溝） — 原因不明、鏡像複製はしない — P2（2026-09-16）

AI照合の指摘13・14（付録A #5、#88）で、**延髄が左側のみ**・**後横側副溝が
右側のみ**の独立ラベルを持つことが分かりました。原因（側タグの誤り／反対側の
欠落／別名への統合／抽出漏れ）はレビュー本文からは判定できません。
