...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-197〜F-206 / L-95〜L-104** | 失語・言語経路モデルの再設計（使うのは F-197〜F-202 と L-95〜L-97。**採番し直し 3 回目**: F-167 → F-177 → F-187 → F-197、L-60/61 → L-81 → L-95。3 回とも main が先に取っていました——下の 16 回目参照） | 2026-09-22 |
...
- **重複の一覧は [`tests/follow-ups-numbers.test.js`](../tests/follow-ups-numbers.test.js) が持ちます。ここには書き写しません。**
  ...
  **15 回目（この PR）** 13 回目に F-167→F-177 と動かした側が、**その F-177 も
  取られていました**（14 回目の branch が同じ日に F-177〜F-186 を予約）。
  L-60 / L-61 も同様に main が使用済みで、F-187〜F-196 / L-81〜L-90 へ動かしました。
  **14 回目が「L にブロック予約が無いことが原因」と書いたので、この PR は
  L のブロックも予約しています。** 2 回とも原因は同じで、**採番してから
  マージ直前までに main が進んでいたこと**です。
  ...
  **16 回目（この PR、マージ直前）** 15 回目に F-177 → F-187 / L-60・L-61 → L-81 と
  動かした側が、**その F-187 と L-81 も取られていました**——心拍出量の branch が
  同じ日に F-187〜F-196 と L-81〜L-90 を両方ブロックで予約しており、
  main へ先に入っています。F-197〜F-206 / L-95〜L-104 へ動かしました。
  **同じ branch が 3 回連続で踏んでいます。** 原因は 3 回とも同一で、
  **shallow clone のまま採番し、マージ直前まで `origin/main` を fetch しなかったこと**
  です。14 回目が示した「push 直前にもう一度 fetch」は今回も効きました
  ——効かなかったのは**着手時の採番**のほうで、`git fetch origin main` を
  **予約行を書く前**に必ず実行してください
  （`git rev-parse --is-shallow-repository` が `true` の clone では、
  `origin/main` は取得するまで古いままです）。
...
### F-164 学習モジュールを持っていない — P3（2026-09-20、2026-09-21 更新）

`higher-brain-function` は `getLearningModules()` を実装していません。
**予測 → 操作 → 説明**にちょうど向いた主題です（「弓状束だけを切ると
復唱はどうなる？」は予測させる価値があり、答えはモデルから再導出できます）。

- **Reel は 2026-09-21 に入りました**（15 秒・4 連続試行）。残りは学習モジュールだけです
- **やるなら**: `tests/learning.test.js` が教材の正解をモデルから再導出して
  照合するので、そこに載せる形で書くこと

### F-201 `higher-brain-function` の viewport 検査 53 件 — 同条件で base と比べた結果、今回の回帰は 0 — P2（2026-09-22）

前回は「このシーンで 13 件、公開シーン `brain-anatomy` で 1 件」とだけ書きました。
**画面の差は示しますが、今回の差分が原因でないことは示しません。**
同条件で測り直しました。

| 何を固定したか | 値 |
| --- | --- |
| 検査スクリプト | `scripts/check-viewports.mjs`（**HEAD の 1 本**を両方に使用） |
| base | `bc88fca`（AR2 前の監査 HEAD）を `VITE_ALLOW_PREVIEW=1` でビルド |
| head | `838ef44`（AR2-01〜09 後）を同じ手順でビルド |
| surface / scene | `--scene higher-brain-function --preview` |
| viewport | 既定の全 viewport（phone-320 〜 desktop-1280） |
| browser | 同一 Chromium・software GL・`deviceScaleFactor` 既定 |
| 走らせ方 | **1 本ずつ**（L-13 の 2 回目参照） |

生データは [`artifacts/ar2-ui-comparison/`](../artifacts/ar2-ui-comparison/)
...
| 新規回帰 | **0** | — |
| 既存・悪化 | 0 | — |
| 既存・同等 | 52 分類 | stage stepper のボタン（`button.step` 24px）、操作バー（`button.btn`）、alpha バッジと `title` 属性の英語、`button.related-toggle` 32px、観察パネルの重なり |
| 原因未確定 | 1 分類 | phone-430 の 44px 未満一覧で、base に出ていた `button.related-toggle` が head では出ず、代わりに `button.btn` が 1 つ増えています。総数は変わりません。参照パネルが上に入って `related` の折り返しが変わった可能性が高いものの、**要素単位で確かめていません** |

- **今回足した操作部は 1 つも 44px 未満の一覧に出ていません**——`.reference-toggle`・
  `.reference-entry`・`.reference-close`・`.metric-group-toggle`・
  `.metric-details-toggle`。節見出しと詳細ボタンは最初 32px で、
  `verify:disease` が落として 44px にしました
- **やるなら**: 残る 52 分類は全部共有 chrome です。操作バーのボタンと
  alpha バッジの英語属性は全シーンに及ぶので、「1 件直したら 1 本出す」に従って別 PR
- **やってはいけないこと**: このシーンだけに例外 CSS を書くこと
- **測れていない面**: ラベル層は `position: fixed` のオーバーレイなので
  `scrollWidth` に出ず、はみ出し検査に**かかりません**（既存・L-54 の形）。
  パネルの**読める幅**も自動では測っていません（L-99）

### F-202 読み取り欄のグループ化と開閉 — 入れた。残りは幅の自動測定 — P3（2026-09-22）

**入れたもの**: 読み取り欄を 4 節にしました——操作しているもの／辿っている課題／
この結果が決めていないこと／比較のための他の課題。比較の節だけ畳み、
**「このモデルが経路を持たない課題」は開いたまま**にしています
（「値が無い」ことが所見なので、畳んで隠すと読者はモデルが答えたと思います）。
制約は 1 件＋件数を出し、`details` のコントロールから**全件**開けます。
狭い画面で見出し行以外を落とす規則があるため、限界を述べる行には
`essential` を付けて残しました——**手に持っている画面でしか見えない限界は、
その製品には無い限界**だからです。

- **残っていること**: パネルの**読める幅**を自動では測っていません（L-99）。
  390px では左の列が 110px しかなく、参照パネルは開いたときだけ
  シートになります。読み取り欄そのものは右の rail にあり、絵では確認済みですが
  検査は「はみ出し 0px」しか見ていません
- **やるなら**: `verify:ui` に「パネルの内容が折り返せる最小幅」を足す。
  F-201 の共有 chrome の件と同じ PR に入れるのが自然です

### F-198 概念的な連絡と実トラクトの線種 — 入れた。残りは印刷・色覚での確認 — P2（2026-09-22）

**入れたもの**: 経路は連絡ごとのメッシュに分かれ、`mapping` で線種が変わります
——実トラクトのメッシュは**実線**（半径 0.024）、粗い代用は**長い破線**
（4 on / 3 off, 0.019）、対応する構造を持たない連絡は**点線**（1 on / 3 off, 0.014）。
隙間は頂点のアルファ 0 で、色ではなく**線そのもの**が違います。凡例は 3 行になり、
どれが何についての主張かを書いています。構造を持たない段階は
**位置も無かった**ので線から落ちていました（描かれる経路がモデルの解いた経路より
2 段階短かった）——いまは隣り合う段の中点を脳の外へ押し出した
**模式位置**に置き、その座標は solver に戻りません
（`AR2-T21` / `AR2-T34` が固定）。

- **残っていること**: (a) 白黒印刷と色覚特性での見分けを**見ていません**。
  線種は色に依存していませんが、実際に確かめたのは画面 1 種類です。
  (b) 概念モードのときに 3D をどう変えるかは**まだ決めていません**
  ——読み取り欄はモードを言い、絵は同じです
- **やるなら**: `shots:anatomy` / `shots:reel` の出力をグレースケール化して見る。
  1 コマンドで足せます

### F-199 古典症候の参照パネル — 画面に出した。残りは幅の自動測定 — P2（2026-09-22）

**入れたもの**: `src/components/ReferenceLibraryPanel.js`。8 症候＋3 つの
「失語ではない」像を**1 件ずつ**開き、各項目が「典型的にどう記載されるか」と
**このモデルが評価していないもの**を並べます。一致率・確信度・順位付けは
ありません（`AR2-T33` がデータ側に `match` / `score` / `probability` /
`rank` が無いことを固定）。キーボードで開閉でき、Escape は項目 → パネルの順に
閉じ、**フォーカスは開いたボタンへ戻ります**。390px では開いたときだけ
chrome と console のあいだのシートになります。solver からは引き続き
import できず、テストは「表示のための import」と「計算での使用」を
区別する形に書き換えました。

- **残っていること**: パネルの**読める幅**は自動で測っていません（L-99）。
  最初の実装は 44px の床を全部満たしたまま**幅 110px の柱**で、
  絵を開いて初めて分かりました
- **やってはいけないこと**: 結果に応じて症候を並べ替える・点数を出す。
  それは外した分類器を別の帽子でかぶせることです

### F-200 書字 2 経路の局在 — 主張を下げた。本文は未取得のまま — P2（2026-09-22）

語まるごとの綴りを優位側角回、音韻—文字変換を優位側縁上回に置いています。

**今回やったこと**: 局在の主張を下げました。両ノードのメッシュは脳回まるごとで、
`omits` に「後方角回と前方角回を区別しない」「縁上回まるごとで、出典が併記している
島は含まない」と書いています。`sourceVerification` は
`via-review-material`（レビュー資料経由）で、`claimSupport` は
`source-is-narrower`——公開 SUMMARY が音韻性失書群の病変を
**縁上回またはその深部の島**を含むものとして記載しており、
1 つの脳回への割り当てより広い条件だからです。
**機能としての 2 経路の解離は別の主張として残しています。**

- **確かめていないこと**: 本文未取得（この環境から医学出版社に到達できません。
  レビュー側が SUMMARY を読んだと書面で述べているだけで、
  **こちらが一次資料を直接確認したことにはなりません**）。
  後続研究（Rapcsak 2007 ほか）の書誌・課題・病変範囲も未照合
- **やるなら**: 本文が読める環境で照合し、`sourceVerification` を
  `body-passage` へ上げるか、局在を概念ノードへ落とす
- **いま書いてあること**: evidence registry で `uncertain` ＋
  `via-review-material` / `source-is-narrower`、model card の §10 と dossier §3

### F-204 3D の線種が reel で読めない — 閉じた（2026-09-22）

**解決しました。** 線の太さと破線周期を world unit から
**提示されるフレーム高さに対する割合**へ移しました。カメラ 1 つで決まり、
画像を一様に拡縮しても変わらず、devicePixelRatio を見る必要がありません。

| 条件 | 実測 |
| --- | --- |
| reel 最終フレーム 675×1200 DPR1、10.2 秒（F-204 を測ったのと同じ条件） | 実線 3.2px / 長い破線 2.4px・12px on・8px off / 点線 1.7px・2px on・8px off。**3 種類とも見分けられます** |
| Desktop 1440×900 DPR2 | 同上 |
| Phone 390×844・375×667 DPR2 | 同上 |

画像は [`artifacts/ar3-line-types/`](../artifacts/ar3-line-types/)
（1:1 の切り出しを含む）。テストは AR3-T12〜T14b、教訓は L-101。

- **残っていること（F-198 へ移します）**: 白黒印刷と色覚特性での見え方。
  線種は色に依存せず**太さでも 2 倍近く違います**が、確かめたのは画面 1 種類です
- **残っていること**: 動いているときのちらつき。確かめたのは静止画です

### F-205 限界を述べる行は「データ」ボタンの向こうにある — P3（2026-09-22）

読み取り欄は `#ui[data-view='learning']` で既定では `display: none` で、
「データ」を押すと出ます。**これは今回の改修より前からの製品の作りです**
（`hasDataOnlySurface()`）。AR2-06 §9.2 の「実コントロールから全件へ到達できる」は
満たしていますが、**到達に 2 手**かかります——データ → 詳細。

- **なぜ気になるか**: 「この値が決めていないこと」は値と同じ画面にあるべきもので、
  いまは値そのものも同じ場所にあるので矛盾はありません。ただし
  **最初に見えているのは 3D と stage 文だけ**で、stage 文は限界を述べません
- **やるなら**: 学習ビューでも「辿っている課題」の節だけ出す、
  という案があります。全シーンに及ぶので別 PR
- **確かめたこと**: `verify:disease` の開閉検査は、自分で「データ」を
  押してから測るようにしました（前の手順が押していたから通っていた、
  という状態を残さないため）。同じ検査が、**課題ボタンを実際に押して**
  読み取り欄の主結果が付いてくることも見ます（AR3-01）——こちらは
  モデル操作パネルが出ている幅で測ります。phone では
  `.model-controls:not(.is-primary)` が隠れるためです

### F-206 線種は静止画でしか確かめていない — P3（2026-09-22）

F-204 を閉じた根拠は**静止画 4 条件**です（`artifacts/ar3-line-types/`）。
確かめていないのは次の 2 つで、どちらも「見え方」の要件の一部です。

- **動いているときのちらつき。** 破線は tube のリングに載っており、
  カメラが動くと見かけの大きさが約 6% 刻みで再生成されます。静止画では
  分かりません。**15 秒シーケンスを通しで見る**のが確認方法です
- **白黒と色覚特性。** 線種は色に依存せず、太さも 3.2 / 2.4 / 1.7px と
  違いますが、確かめたのは画面 1 種類です。
  `npm run shots:anatomy -- --cvd <kind>` が解剖シーン向けに
  同じ枠組みを持っています——病態シーンでも使えるようにするのが筋です

- **やるなら**: `shots:reel` の出力をグレースケール化／CVD 変換する
  オプション 1 つ。F-198 の同項目をここに寄せました

### F-203 左の列のパネルは、phone で読める幅を持っていない — P2（2026-09-22）

F-201 の測定中に測ったもので、**今回の改修が原因ではありません**。
390×844 で `.top-left` は **110×169px** です。その中で:

| パネル | 390px での状態 |
| --- | --- |
| `.model-scope` | **0×0**。モデルの範囲と出典が、phone では画面に存在しません |
| `.related-scenes` | 幅 110px。`button.related-toggle` は 32px 高（44px 未満、F-201 の一覧に出ています） |
| `.reference-library` | 開くとシートになる（今回入れた）。**閉じているときは同じ 110px の柱** |

- **なぜ問題か**: 「このモデルが答えないこと」を読める場所が、
  訪問者の大半が持っている画面に無いということです。
  `docs/architecture/intended-use-and-model-provenance.md` の契約は
  「範囲は同じ画面にある」ことを前提にしています
- **やるなら**: 3 つとも同じ扱い——閉じているときは列の中、開いたら
  chrome と console のあいだのシート。`.reference-library` の
  `@media (max-width: 860px)` の規則がそのまま雛形になります
- **ついでに揃うこと**: 新しいパネルは 12px（型の床）で、隣の 2 つは 9〜10.5px です。
  床は縮む方向にしか動かせないので（`npm run type-floor`）、
  シート化と同時に 2 つを床まで上げるのが自然です
- **やってはいけないこと**: phone で `display: none` にして「対応した」とすること

### F-197 15 秒シーケンスの実レンダリング — 見た。残りは 2 点 — P3（2026-09-21）

Reel を 1 病変（弓状束）から **4 連続試行**（無傷 → 前 → 後 → あいだ）へ書き換え、
**`npm run shots:reel` を作って実際に描きました**（新規: `scripts/capture-reel-frames.mjs`。
どのシーンの reel でも、秒を指定して静止画にできます）。

見て分かったこと 2 件は**この PR で直しました**——(1) clean mode がグローバルナビを
隠しておらず、**全シーンの動画にパンくずとログインが写っていた**
（`docs/verification-lessons.md` L-95）、(2) 「どこで止まったか」が
**小さなマーカー 1 個**にしか出ておらず、前病変と後病変の絵が見分けられなかった
（`docs/organ-3d-playbook.md` M3。いまは経路線そのものが明暗で語ります）。

- **まだ確かめていないこと**: (a) 1 試行 3.4 秒が「速すぎて読めない」かどうか
  ——静止画では分かりません。動画として通しで見る必要があります。
  (b) キャプションが phone 幅で折れる位置（1500×2000 では 2 行、
  実際の 9:16 では 2 行に折れているのを確認済みですが、最小幅では未確認）
- **やるなら**: `verify:ui` の surface に reel を足せないか。main 側が同じ穴を
  **F-170** として既に持っています（F-162 とも同じ形）——3 件を 1 つにまとめる
...
- **F-162 `verify:ui` が非公開シーンを見られなかった** — 解決（2026-09-22）。
  `SURFACES` が scene 面を `#/brain-anatomy` 1 本で代表していたため、公開して
  いないシーンは locked ページとして測られ、**そのシーン自身のレイアウトは
  どの幅でも一度も測られていませんでした**（F-162 に書いたとおり、実際に
  見つかった 1 件は絵で見つけています）。`verify:ui -- --scene <slug> --preview`
  で scene 面を別のモデルに向けられます。初回の実行で
  `higher-brain-function` に 13 件（公開シーンは同条件で 1 件）——内訳と
  帰属は F-201 に。
- **F-165 回路の「連絡」に左右が無かった** — 解決（2026-09-22）。連絡が id で
  指定される別の damage チャネルを持っていたため、**id に左右が無く**、片側の
  内包を選ぶと 3 つの前頭葉回路が両側同時に 0 になっていました。連絡はノードと
  同じ規則で、**自分が走る構造から**（走る側の構造から）健全性を取ります。
  `connections` を宣言する病変は例外を投げます——黙って無視すると、その病変が
  無害に見えてしまうため。併せて `thalamocortical-disconnection` を片側
  （`dominant-anterior-thalamic-radiation`）にしました。ガードは
  `physiology: a unilateral lesion does not cut the other side’s connection` と
  `contract: the old connection channel is refused rather than ignored`、
  ミューテーション 2 件で赤を確認。
