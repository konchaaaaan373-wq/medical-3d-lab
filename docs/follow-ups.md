...
- **番号は、着手時に「ブロック」で予約します。** 最大値 +1 を 1 つ取る方式は、
  ...
  **予約中のブロック**（終わったら行ごと消す）:
  ...
  | **F-213〜F-222 / L-122〜L-131** | ヘッダーとサイトメニューの再構成（臓器 → 層の 2 段ナビ・全画面共通の ☰・言語／アカウント／ご意見のヘッダー集約）。使うのは F-213〜F-216 と L-122〜L-124。`origin/main` を fetch して最大 F-212 / L-121 を確認してから取りました | 2026-09-26 |
...
- **F-209 モデル strip が、解剖と機序を同じ一列に、不揃いな長さで並べる** — 解決（2026-09-26）。
  strip を**臓器 → 層の 2 段**にしました。1 段目は公開している臓器を 1 語ずつ
  （脳・心臓・肺・肝臓）、2 段目は**いま見ている臓器に 2 件以上あるときだけ**その層
  （`解剖` ／ `機序 心拍出量`）。層は名前からではなく **model profile の
  `mechanismLevel`** から引きます（`none` なら解剖、それ以上なら機序、疾患なら病態。
  `publicManifest.js` の `layerOfScene` と行の `layer`）。形は
  `src/app/modelNavigation.js` の `organLayerNavigation` が 1 か所で持ち、
  ヘッダーとサイトメニューの両方がそれを読みます。1280×860 で 154 px あった
  「触れて学ぶ心臓の解剖」は chip から消え、390 px でも 4 臓器が 1 行に収まります
  （肺・肝臓が端から落ちていた件も同時に解消）。
  ガードは `tests/scene-model-strip.test.js`（臓器ごとに 1 chip・名前は臓器名・
  現在ページはちょうど 1 つ・層の行は 2 件以上のときだけ・機序には層名が付く）と
  `tests/model-navigation.test.js`。ミューテーション 11 件で赤を確認済み。
...
> **2026-09-26 追記**: ワードマークは `brandMark()`（`ShellHeader.js`）1 つになり、シーンも `M/3` を着ています。ヘッダーの区画・☰ も全画面で共通です（F-213）。
...
> **2026-09-26 追記**: drawer は全画面共通のサイトメニュー（☰）に置き換わりました。お気に入りは、行が到達できないモデルがあるとき（preview unlock）にメニューのカタログ節に出ます。β の公開面で届かないのは変わっていません。
...
3. **電話での心臓の大きさ**（2026-09-26 に作り直し後の実測、preview ビルド）: パッドは
   1 つずつ（切替）、縦長の画面では枠をループ上端でなく心臓の少し上で止める。左室は
   390×844 で約 140 px、390×664 で約 82 px（以前 70〜75 px）。preview はビルド表示に
   34 px 取る（`body.has-build-marker`）ので本番ではその分広いが、**それで解決済みとはしない**。
   `verify:disease` がこの値を下限として強制する（悪化の検出であって十分の証明ではない）。
   **375×553 は成立していない**（心臓の帯が約 34 px）。サイトのナビ・タイトル・計算結果・
   操作欄のどれかを畳む必要があり、どれを畳むかは製品判断——**所有者の判断待ち**
4. **縦の range が短い**（短い電話では 0〜40 px）。y 軸は両端のボタンと点のドラッグが主で、
...
### F-213 — ヘッダーとサイトメニューは headless Chromium でしか見ていない — P2

2026-09-26、全画面のヘッダーを同じ 4 区画（ロゴ＝ホーム／その画面のナビ／言語・
アカウント／☰）に揃え、☰ を全画面に置きました。**確認済み**: 単体テスト、
`verify:ui`（全 viewport × 全 surface。新設の `checkSiteMenu` がパネルの位置・重なり・
フォーカス・Escape を測る）、`verify:departure`（10 回の遷移のあともアカウントボタンは
1 つで、押せる）、1440 / 1024 / 768 / 390 / 360 の実レンダリング（本番ビルドと
preview ビルド）。全行の初回で出た 19 件（320・375 でロゴが 41 px 幅）は直して
その行を再実行し緑。**1 件だけ再現していません**: 全行実行の 768 px の 3D で一度
「Escape でメニューが閉じない」と出て、同じセルの単独実行 2 回と 768 行の通し実行
1 回では出ませんでした。software GL の描画が主スレッドを塞いだ可能性が高いと見て
待ちを 2 → 5 秒にし、再発したら何がフォーカスを持ち、他に何が開いていたかを
名指しするようにしてあります。`verify:anatomy` の赤は 390×844 の拡大の支点
（21 px / 戻して 18 px）の 2 件だけで、**`origin/main` のビルドで同じ数値**が出ます
——この変更由来ではなく、F-197 が既に持っています。**未確認**:

1. **実機**（iPhone Safari・Android Chrome）: `popover`（top layer）と `::backdrop`、
   パネル位置を JS で測って置く処理がソフトウェアキーボード・ツールバー伸縮・
   safe area でずれないか
2. **Safari / Firefox**: `@supports selector(:popover-open)` の分岐（未対応なら
   従来の固定配置 + ヘッダーの z-index で代替）が実際に選ばれるか
3. 心臓のページだけ電話でヘッダーが 2 段（約 +48 px）になる。3D の帯が縮む量は
   F-212 の 3 番（375×553 は成立していない）と同じ場所の問題なので、そちらと一緒に測る

*この PR の実施者が、マージ前に記録したもの。*

### F-214 — 臓器 chip の `aria-current="true"` と、層 chip の `"page"` の読み上げ — P3

臓器の下に層があるとき（心臓）、臓器 chip は「その中にいる」を `aria-current="true"`、
層 chip は「このページ」を `"page"` で言います。層が 1 つだけの臓器では臓器 chip が
`"page"` です（`tests/scene-model-strip.test.js` が固定）。**F-147 と同じ語彙の判断**
で、実機のスクリーンリーダーでどう読まれるかは聞いていません。
F-147 のフッタ（`モデル一覧` に `page`）は、いまは **preview unlock のときだけ**
描かれる（β ではメニューにカタログが出ない）ので、β の公開面の範囲からは外れました。

### F-215 — ヘッダーの「区画」は揃ったが、左端の位置は面ごとに違う — P3

区画の並び・語・メニューは全画面で同じになりました。**左端の x は揃っていません**:
ランディングは 1400 px の列の中（左 24 px の余白 + ヘッダー自身の余白）、
公開とレビュー・規約は画面の端から、準備中はカードの列の中、3D は浮いたバー。
全幅に揃えるならランディングの列の外にヘッダーを出す必要があり、ページの構成の
変更になるので**製品判断として残しました**。

### F-216 — 「機序」という語を公開面に出した — 医学的な語の確認が未了 — P2

`cardiac-output` を「機序 心拍出量」と表示しています（英語は Mechanism）。
語の出所は model profile の `mechanismLevel: mechanistic` で、病態（disease）と
区別するためです。**「生理」「しくみ」ではなく「機序」でよいか**は医学レビューの
判断で、まだ受けていません。変えるなら `MODEL_LAYERS`（`publicManifest.js`）の
1 か所で、ヘッダー・メニュー・カタログの 3 面が同時に変わります。
