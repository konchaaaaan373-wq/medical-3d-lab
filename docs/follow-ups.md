...
- **床はあえて低くしてあります（2026-09-20 改訂）。** 一度 4 視すべてで ΔE 12 以上まで
  上げましたが、**中心溝（前頭葉↔頭頂葉）が 18.7 に落ちて読めなくなり**、
  差し戻しました（[L-52](verification-lessons.md)）。いまの床は**表面 8 family で
  ΔE 4**、実測 4.9 / 4.7 / 5.8 です。**4〜5 は「違う」であって「読みやすい」では
  ありません**——部分的な措置です
- **いちばん弱い組**: protan の前頭葉↔頭頂葉（ΔE 4.9）と deutan の
  辺縁葉↔脳幹（4.7）。P/D 型に残る色度軸は青–黄の 1 本で、**暖色どうしは
  明度でしか離せない**ためです
