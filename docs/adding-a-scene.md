...
## 9. モデル層を持つシーン（`alpha` 以上）

`prototype` は形と動きのスケッチです。そこから先へ進むシーン
—— 数値を出し、グラフを描き、「この関係はこうなっている」と主張するシーン ——
は、**4 つのものをセットで持ちます**。どれか 1 つでも欠けていれば、
そのシーンは数値を出す資格がありません。

### ① `src/models/<subject>.js` — 医学モデル層

純粋な JavaScript です。`three` も DOM も `window` も import しません。
`node --test` だけで検証できることが条件で、これは形式的な要求ではありません。
**レンダラーを起動しないと確かめられない生理学的主張は、確かめられていない**
のと同じです。

- 臨床的な単位・臨床的な名前（`endExpiratoryVolumeL`, `portalPressureGradientMmHg`）
- 演出値は入れない。opacity も glow も誇張係数もシーン側
- 決定論的であること。不均一性はシードで固定する
  （リロードのたびに違う肺では教えられません）
- 積分するものは [`integrate.js`](../src/models/integrate.js) の固定タイムステップを使う

詳細は [`src/models/README.md`](../src/models/README.md)。
...
| **Claim** | モデルが主張していること |
| **Source** | それがどこから来たか |
| **Implementation** | コードのどこでどう実装されているか |
| **Assumption** | そのために何を仮定したか、何を落としたか |
| **Validation** | それを固定しているテストの名前 |

**出典に到達できなかった場合は、そう書いてください。** 要約や抄録しか
読めていないなら「本文は参照していない」と明記し、定量的な裏付けが
取れなかった行には **thin** の印をつけます。読めていない論文を
読んだかのように引くことが、このドキュメントで唯一やってはいけないことです。

### ③ `docs/model-cards/<scene>.md` — 境界

16 節の定型です。答える問い、答えない問い、入力、出力、支配方程式、
定数の出どころ、**較正値と実測値の区別**、誇張しているものとしていないもの、
既知の failure mode、絶対に使ってはいけない用途、不確実性、
誤解を生みうる場所、レビュー状況、確かめ方。

いちばん重要なのは「**Known failure modes**」と「**Where the model could
mislead**」です。ここが薄いモデルカードは、書く価値がありません。

### ④ scope panel — 同じ画面の上に

`meta.modelScope` を宣言すると、3D の横に折りたたみパネルが出ます
（[`ModelScopePanel.js`](../src/components/ModelScopePanel.js)）。
...
- **`getCharts()` と `meta.charts`** — プロットの静的な部分（題・軸・凡例）は
  `src/data/` に、数値だけが毎フレーム来ます。プロットは
  「モデルが計算過程を見せている場所」であって、モデルをよく見せる場所では
  ありません
- **`getCausalStory()`** — 因果の連鎖を 1 リンクずつ辿る walk-through。
  ステップには必ず `because`（前のステップからどうつながるか）を書きます。
  `because` が書けないステップは、因果の連鎖の一部ではありません
- **`getLearningModules()`** — predict → manipulate → observe → explain
  （必要なら → transfer）。**保存された答えは必ずテストでモデルから
  再導出してください。** 文章は下手でも構いませんが、モデルの挙動について
  間違っていてはいけません
- **`settleModel()`**（任意）— 平衡に達するまでに時間がかかるモデル用。
  スライダーを動かした 3 秒後の値を「答え」として測ると、過渡状態を
  結果として報告することになります

### なぜ `production` まで上げないのか

`production` の条件には **④ 臨床の目でレビューを受けた** が含まれます。
これは飛ばせません。モデル層・dossier・model card・テストがすべて揃っていても、
臨床レビューを受けていないシーンは `alpha` です。バッジを外すというのは
「ここに書いてある数字は信じてよい」と言うことなので、
それを言えるのはレビューを受けたあとだけです。
