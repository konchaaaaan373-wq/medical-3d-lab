...
- **心臓 driver の 3 viewport 連続実行**は、この環境で落ちていました。切り分けた結果、
  **原因は 5 つ**あり、**4 件は driver 側の欠陥で修正済み、1 件は原因未確定・未完了**です。
  最初に「容器の資源」と報告しましたが、**それは裏付けの無い推測**でした。
  ...
  3. 3 つ目の scene が 180 秒で `ready` にならない → **原因未確定・未完了**。
     `exit 2` へ変えたのは報告分類の修正であって、**解消ではありません**
