...
- **臨床ケース学習**: 症例を題材に機序を確認するところまで。治療推奨・診断・
  重症度判定・意思決定支援や、ドブタミン（DOB）などの患者別用量調整はしない
- **患者説明のバッジとフィルタは fail closed**: 有料の患者モードと同じ条件
  （reviewed 以上、かつ現行系統の版固定レビュー完了）を満たすまで表示しない。
  製品内の用途名は「患者説明 / 医学教育 / 臨床ケース学習」の 3 つに固定し、
  「臨床応用」は現時点の機能名として使わない
