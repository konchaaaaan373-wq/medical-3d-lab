...
| --- | --- | --- |
| **患者説明** | 平易な言葉と絞った操作で、一般的な機序を会話に使う | 患者データ入力、個別予測 |
| **医学教育** | 予測 → 操作 → 観察 → 説明、比較、教材 | モデルの範囲を超える一般化 |
| **臨床応用** | 症例ベースの機序確認（clinical case learning） | DOB などの患者別用量調整、治療推奨、意思決定支援 |

患者別の用量調整や治療推奨は、根拠・適応範囲・検証・変更管理・臨床レビューを
備えた**別の検証済み製品**として判断する。現行 Explorer の用途ラベルは、
その機能が有効であることを意味しません。
...
| lungs | A2 + 部分 A3 | 5葉・18区域・区域気管支、伴走する肺動脈と区域間肺静脈を `lung-anatomy.test.js` で固定。胸膜は未実装 |
| liver | A2 | Couinaud区域、門脈枝と肝静脈の関係を `liver-anatomy.test.js` で固定 |
| 残り 18 臓器 | A0–A1 | 多くはスタイライズドな外形。pull が来るまで区画を増やさない |
