...
neither is publishable (F-109). What B3 did find in the shared layer was the
...
outside the band (F-108). **Three batches, three defects in shared code that
...
| B3 | `skin-anatomy` | **Open, 2026-09-14.** Began as `eye`, `ear`, `skin`; the renders found the safe-area fit was cutting subjects off, an arteriole hidden behind its venule, and a viewpoint named for structures it did not show (F-107, F-108) |
| — | `eye-anatomy`, `ear-anatomy` | Held out of B3 on what their renders showed, not on the gate (F-109) |
