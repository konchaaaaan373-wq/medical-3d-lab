...
| **Held** | `eye-anatomy`, `ear-anatomy` (F-132) |
...
| **`fit/after/`** | three published scenes re-framed by the fit correction (F-131) |
| **`held/`** | why the eye and the ear are not in this batch |
| **`framing-at-load/`** | a discrepancy found while fixing the check (F-133) |
| **`eye-after/`** | three of the eye's four defects, fixed (F-132) |
...
solving the projection instead (F-131).
...
second. It was two defects on top of each other (F-133), and both are fixed:
