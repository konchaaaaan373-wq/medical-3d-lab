...
> 糸が密に集まり、細胞と細胞のあいだに沈着します。この沈着は、顕微鏡でこの病気と判断するときの所見のひとつです。
...
### 7. この流れが原因かどうかは、まだ議論されています

> この蓄積が症状を引き起こすという考えは、研究者が挙げている説明の一つです。異なる立場もあり、決着はついていません。

- **画面のどこを見るか**：この画面が示すのは形の移り変わりだけです。誰かが不調になった理由を示すものではありません。
- **区分**：**educationalOnly（モデル出力ではありません）** ／ 確からしさ：**仮説（決着していない）**
- **モデルの状態**：`plaque`（progress 0.84）
- **専門家表示の同じ段**：**プラーク（老人斑）形成** — 線維が密に集まり、細胞外に老人斑（プラーク）を形成します。アルツハイマー病の神経病理学的特徴のひとつですが、この状態でもモノマーや可溶性凝集体は併存しています。
