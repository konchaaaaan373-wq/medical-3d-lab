...
- **壊れた計器は直す。** チェックが恒常的に赤い／原因を誤って報告しているのは、
  それ自体が他の全部の信号を殺すので本質的です（F-108 はこれ）
...
### 教訓は md に書く。同じ失敗を二度しない

**間違いを 1 つ見つけたら、それを直す PR の中で、所有文書に 1 項目書いてから
次へ行く。** 別 PR に回さないでください。回した教訓は書かれません——この repo で
「あとで書く」と言った教訓のうち、実際に書かれたものは 1 件もありません。

書く先は、その失敗が何の失敗だったかで決まります。

| 何を間違えたか | 書く先 |
| --- | --- |
| **検証そのもの**が嘘をついた（緑なのに何も測っていなかった） | [`docs/verification-lessons.md`](docs/verification-lessons.md) |
| 3D シーンの**構造**が静かに壊れた | [`docs/architecture-rules.md`](docs/architecture-rules.md) |
| **レンダリング**の失敗モード | [`docs/organ-3d-playbook.md`](docs/organ-3d-playbook.md) |
| 医学的に**単純化**した | [`docs/medical-notes.md`](docs/medical-notes.md) |
| マージしたが**未確認・未決定** | [`docs/follow-ups.md`](docs/follow-ups.md) |
| 上のどれでもない | まず `docs/verification-lessons.md` に置いて、持ち場が見えたら移す |

**自己改善ループは 5 手です。**

1. **ガードを書く** — テスト・検証スクリプト・アサーション
2. **壊す** — 修正を戻す / 条件を反転する / 対象を改名する
3. **赤を確認する** — 赤くならないガードは**存在していません**
4. **戻して緑を確認する**
5. **台帳に 1 項目書く** — 症状・どう見つかったか・いま何が捕まえるか

**3 手目を飛ばさないでください。** `docs/verification-lessons.md` の A 節
8 件は、**全部 2〜3 手目でしか見つかっていません**。どれも書いた時点では正しく見え、
レビューを通り、緑でした。うち 1 件は、テストを追加する編集スクリプトが途中で
落ちて何も書かず、**テストが存在しないまま**ミューテーションが 3 つ素通りしています。

`npm run lessons`（`npm test` からも走ります）が台帳の側を守ります——各項目が
3 つの欄を持つこと、**名指ししたガードが実在すること**、そして
**まだ人しか捕まえられない項目が何件あるか**。その数が下がることが目的で、
**項目を消して下げるのは逆走です。**

### Netlify credits を浪費しない

Netlify の月間 credits には上限があります。**commit の粒度と push の粒度を分けてください。**
ローカルでは小さく commit してよい一方、作業途中の commit ごとに push して
Deploy Preview を連続起動しません。

- 同じ主題の修正・ローカル検証・自己レビューを完了してから、まとめて push する
- push は、レビュー可能な状態になったとき、または実画面・実機確認に Deploy Preview が
  必要な節目に限る。単なる進捗保存のためには push しない
- push 前に可能な検証をローカルで終え、push 後の軽微な修正往復を減らす
- 文書だけの変更など、実画面確認が不要な PR は Preview Deploy を前提にしない
- CI / Netlify の手動 rerun は、失敗原因を確認し、再実行で意味のある結果が得られる場合だけ行う
- credits が逼迫している期間は、不要なブランチ・Draft PR・自動 Deploy の対象を増やさない

これは下の「1 件直したら 1 本出す」と両立します。**PR は小さく保ち、1 PR の作業中に
何度も push しない**という運用です。医学・認証・公開判定など安全性に関わる修正は、
credits 節約を理由に必要な検証を省略しません。
...
公開βは 2026-09 から 2026-09-22 まで解剖層だけを公開していました
（[ADR](docs/architecture/adr-2026-09-08-anatomy-only-beta.md)）。
2026-09-22 に所有者の判断で機序シーンを 1 本ずつ加えられるようになりました
（[ADR](docs/architecture/adr-2026-09-22-mechanism-scene-in-beta.md)）。
**代用の禁止は残っています**——病態は解剖の「代わり」にはなりません。
...
### β が公開するのは、解剖と、名指しした機序シーンだけ

現在は **β 公開中**です。公開するのは**解剖モデル**と、
**`BETA_MECHANISM_CANDIDATES` に名指しで載せた機序シーン**だけ。
それ以外はルートもカタログのカードも「TO BE UPDATED / 準備中」で止めています。

2026-09-22 まで、ここは「公開するのは解剖モデルだけ」でした。所有者の判断で
そのルールを外しています（[ADR](docs/architecture/adr-2026-09-22-mechanism-scene-in-beta.md)）。
**外したのは「解剖だけ」の部分で、それ以外のゲートは 1 つも緩めていません。**

- **代用の禁止は残ります。** 病態シーンを解剖シーンの**代わりに**公開しません
  ——病態モデルはラベルを変えた解剖モデルではないからです。機序シーンが開くのは、
  その臓器の解剖シーンが開いた**あと**であって、その代わりではありません
- **患者向けの機序シーンは、現行の臨床レビューを要求します。** `intendedUses` に
  `patient-explanation` を含む、または患者ビューを持つシーンは、現行レビューが
  無ければ閉じたままです。**β が新たに開けるのは「学習者に向けた機序」**です
- **広いルールではなく、名前の一覧です。** 「記録が揃っている機序シーンは全部開く」
  にはしません。1 本追加するたびに、その 1 本についての公開判断記録が要ります
- **解剖の候補一覧のルールは 1 行も変えていません。** `anatomyClaimProblems` は
  そのままで、そこに機序シーンを載せることはできません

**何件公開しているかは、ここには書きません。** この節は 2026-09-16 まで
「公開しているのは脳と心臓」「現在の公開は 1 件」「`heart-anatomy` はまだ存在しない」
と書いており、**3 つとも間違っていました**。同じ失敗を Explorer の副題もしています
（「目標は脳と心臓の 2 つ」と書いたまま、その上に 3 臓器を並べていました）。
**下の「公開シーンの一覧をどこかに書き写さないでください」は、この節自身にも
...
`tests/beta-release.test.js` が 1 か所で固定し、直し方をメッセージに書きます。

**ゲートが通ることと「見たこと」は別です。** B1（肺・肝・腎）は
`verify:anatomy` が 3 件とも通ったあと、全視点をレンダリングして初めて欠陥が
3 つ出ました。B2 でも同じで、**中空の臓器の断面が「肉の塊」になっていた**ことは
絵を見るまで誰も気づいていません。B3 は 3 件で始めて 1 件になりました——眼と耳は
ゲートを通りますが、眼は開始状態が白い球で、耳の蝸牛はコイルばねです（F-132）。
2026-09-16 にはもう一段あって、**公開中の 4 シーン全部でクリック tour が
壊れていました**（安全域フィットの修正でモデルが動いたため）。テストは
1 件も赤くなっていません。**新しい臓器を公開するとき、また共有コードを
触ったときは、必ず実レンダリングと `verify:anatomy` を見てください**
