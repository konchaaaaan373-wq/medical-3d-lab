...
現在の公開は **11 件**（脳・心／肺・肝・腎／胃・食道・腸・胆道・膵／皮膚）。順番は
...
通りますが、眼は開始状態が白い球で、耳の蝸牛はコイルばねです（F-132）。
**3 バッチ続けて、共有コードの欠陥は絵でしか見つかっていません。**
**新しい臓器を公開するときは、必ず実レンダリングを見てください**
（`npm run shots:anatomy -- --scene <slug> --preview`）。

**心臓の解剖シーン（`heart-anatomy`）は 2026-09-15 に公開に入りました**
——候補 asset が asset release gate を通り、その hash に結びついた公開判断記録が
できた日です。**hero もカタログもクロール面も編集していません**：判定は下の
1 か所が持っているので、ゲートを通れば公開一覧はひとりでに動きます。
**心臓の病態シーンは、これとは別です。** 解剖が開いたことは病態について
何も言いません——病態モデルはラベルを変えた解剖モデルではないからです。
